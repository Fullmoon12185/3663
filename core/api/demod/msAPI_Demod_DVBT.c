...
BOOL msAPI_Demod_DVBT_Init(void)
{
    ...
    #elif(FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
        devDemod_Silab2164B_DTV_Serial_Control(!TS_PARALLEL_DVBT_OUTPUT);    
        devDemod_Silab2164B_DVBTInit();
        MDrv_SYS_SetPadMux(E_TS0_PAD_SET, E_SERIAL_IN);
...
//    devCOFDM_ControlPowerOnOff(bTurnOn);
    ...
    )
    {
        ...
        bTurnOn = bTurnOn;
    #elif(FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)    
        bTurnOn = bTurnOn;  //control by demode driver
...
#elif (FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
    return devDemod_Silab2164B_DVBT_GetPostBER(ber);
...
#elif(FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
    return  devDemod_Silab2164B_DVBT_GetLockStatus(eStatus);
#else
    UNUSED(eStatus);
    return FALSE;
#endif
...
#elif(FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
    if( devCOFDM_DVBT_GetLockStatus((COFDM_LOCK_STATUS)COFDM_TPS_LOCK) == FALSE ) // No TPS Locked    
#endif
    {
        // Speed up
...
#elif(FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
    devDemod_Silab2164B_DTV_SetFrequency((U32)(dwFrequency), eBandWidth, eHpLp,u8PlpID);
#else
...
#elif (FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
    *quality=devDemod_Silab2164B_DVBT_GetSignalQuality();
    return E_RESULT_SUCCESS;    
#elif(FRONTEND_DEMOD_DVBT_TYPE == EMBEDDED_DVBT_DEMOD)
 #if DEMOD_DVBT_USE_UTOPIA
    MS_BOOL bRet=FALSE;
...
#elif (FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
    return (BYTE)devDemod_Silab2164B_GetSNR();
...
#elif(FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
    devDemod_Silab2164B_DTV_Serial_Control(bEnable);
...
FUNCTION_RESULT devCOFDM_DVBT_GetSignalStrength(WORD *strength)
{
    ...
    #elif (FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)
        *strength = devDemod_Silab2164B_DTV_GetSignalStrength();
        return E_RESULT_SUCCESS;
