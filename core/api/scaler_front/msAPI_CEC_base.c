...
void msAPI_CEC_SetDeviceName(MS_U8 u8Device, MS_U8* pName, BOOLEAN bIsDefault)
{
    ...
    else
    {
        if( pName == NULL )
        {
            memset( &(_stMsApiCecInfoList.u8CecDeviceName[u8Device][0]), 0, MAX_CEC_DEVICE_OSD_NAME);
        }
        else
        {
            ...
            Str8_StrCpy_CheckDest((char *)_stMsApiCecInfoList.u8CecDeviceName[u8Device], (char *)pName, MAX_CEC_DEVICE_OSD_NAME);
...
void msAPI_CEC_GetDeviceInfo(void)
{
    ...
    for (i=(int)E_LA_RECORDER1; i<(int)E_LA_MAX; i++)
    {
        ...
        /*
            ...
            Str8_StrCpy_CheckDest( (char *)g_stMenuCecDeviceList.au8CecDeviceName[j] ,
                                    (char *)msAPI_CEC_GetDeviceName((MsCEC_DEVICELA)i),
                                    MAX_CEC_DEVICE_OSD_NAME
                                    );
