...
static void _MApi_XC_Sys_HDMI_Init(void)
{
    ...
    for(i=0; i<HDCP_KEY_SIZE; i++)
    {
        ...
        if(i > HDCP_KEY_CHECK_NUM)
        {
            printf(ANSI_COLOR_RED);
            printf("*** Warning!!! No HDCP key !!\n");
#if (HDCP_KEY_TYPE==HDCP_KEY_IN_CODE)
            printf("*** HDCP key from Code\n");
#elif (HDCP_KEY_TYPE==HDCP_KEY_IN_DB)
            printf("*** HDCP key from DB\n");
#elif(HDCP_KEY_TYPE==HDCP_KEY_IN_EFUSE)
            printf("*** HDCP key from eFuse\n");
#endif
...
#endif
{
    ...
    if (bEnable )
    {
        ...
        if( eWindow == MAIN_WINDOW )
        {
            MApi_XC_SetDispWindowColor(astNoSignalColor[u8Color], SUB_WINDOW);
        }
      #endif
