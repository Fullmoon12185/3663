...
#include "BinInfo.h"
#include "InfoBlock.h"
#define AQ_WORKSHEET_START_MAGIC_NUMBER 0x87598759
#define AQ_WORKSHEET_SIZE 320
#define AQ_SOURCE_SIZE 6 //DTV/ATV/AV/PC/HDMI/MM
#define AQ_HEADER_OFFSET 32 //DTV/ATV/HDMI/PC/AV/MM
...
#if (ENABLE_SUPPORT_AQBIN_FLOW)
ST_AQBIN_PARA stAQBinDTV;
ST_AQBIN_PARA stAQBinATV;
ST_AQBIN_PARA stAQBinHDMI;
ST_AQBIN_PARA stAQBinPC;
ST_AQBIN_PARA stAQBinAV;
ST_AQBIN_PARA stAQBinMM;
extern U16 TvVolumeTable[101];
#endif
...
#if (ENABLE_SUPPORT_AQBIN_FLOW)
#define DEBUG_AQ_BIN(x)  //x

void msAPI_Initial_AQBinPara(void)
{
    U8 u8i = 0;
    ST_SmartSound_Para stTemp = {0, 0, 0, 0, 0, 0};
    ST_AQBIN_PARA stAQBinDefaultValue;
    stAQBinDefaultValue.stAVCPara.u8AttackTime = 0;
    stAQBinDefaultValue.stAVCPara.u8ReleaseTime = 0;
    stAQBinDefaultValue.stAVCPara.u8ClippingLevel = 0;
    stAQBinDefaultValue.u8BassGain = 0;
    stAQBinDefaultValue.u8TrebleGain = 0;
    stAQBinDefaultValue.u8ToneEffectEnable = 0;
    stAQBinDefaultValue.u8MStarSurroundEnable = 0;
    stAQBinDefaultValue.u8DRCEnable = 0;
    stAQBinDefaultValue.u8DRCClippingLevel = 0;
    stAQBinDefaultValue.bHPFEnable = 0;
    stAQBinDefaultValue.u16HPFFrequency = 0;
    stAQBinDefaultValue.u8HPFQValue = 0;
    stAQBinDefaultValue.u8KGain = 0;
    stAQBinDefaultValue.u8LPFGain = 0;
    stAQBinDefaultValue.u8BGain = 0;
    stAQBinDefaultValue.u8AGain = 0;
    stAQBinDefaultValue.u8NRThreshold = 0;
    stAQBinDefaultValue.stDevicePrescale.u8MainSpeaker = 0x66;
    stAQBinDefaultValue.stDevicePrescale.u8LineOut = 0x66;
    stAQBinDefaultValue.stDevicePrescale.u8SifOut = 0x66;
    stAQBinDefaultValue.stPEQ.u8Enable = 0;
    for(u8i= 0; u8i < 8; u8i++)
    {
        stAQBinDefaultValue.stPEQ.u16Fc[u8i]   =0;
        stAQBinDefaultValue.stPEQ.u8Gain[u8i] =0;
        stAQBinDefaultValue.stPEQ.u8Qvalue[u8i] =5; //range 5~150
    }

    memcpy(&stAQBinDefaultValue.stStandard, &stTemp, sizeof(ST_SmartSound_Para));
    memcpy(&stAQBinDefaultValue.stMovie, &stTemp, sizeof(ST_SmartSound_Para));
    memcpy(&stAQBinDefaultValue.stSports, &stTemp, sizeof(ST_SmartSound_Para));
    memcpy(&stAQBinDefaultValue.stUser, &stTemp, sizeof(ST_SmartSound_Para));
    memcpy(&stAQBinDefaultValue.stVivid, &stTemp, sizeof(ST_SmartSound_Para));
    stAQBinDefaultValue.u8SRSonOff = 0;

    stAQBinDefaultValue.stSRSPara.u8InputGain = 0;
    stAQBinDefaultValue.stSRSPara.u8OutputGain = 0;
    stAQBinDefaultValue.stSRSPara.u8DCGain = 0;
    stAQBinDefaultValue.stSRSPara.u8TrubassGain = 0;
    stAQBinDefaultValue.stSRSPara.u8SpeakerSize = 0;
    stAQBinDefaultValue.stSRSPara.u8Definition = 0;
    stAQBinDefaultValue.stSRSPara.u8SurroundLevel = 0;
    memcpy(stAQBinDefaultValue.u16VolumeTable, TvVolumeTable, sizeof(U16)*101);

    memcpy(&stAQBinDTV, &stAQBinDefaultValue, sizeof(ST_AQBIN_PARA));
    memcpy(&stAQBinATV, &stAQBinDefaultValue, sizeof(ST_AQBIN_PARA));
    memcpy(&stAQBinHDMI, &stAQBinDefaultValue, sizeof(ST_AQBIN_PARA));
    memcpy(&stAQBinPC, &stAQBinDefaultValue, sizeof(ST_AQBIN_PARA));
    memcpy(&stAQBinAV, &stAQBinDefaultValue, sizeof(ST_AQBIN_PARA));
    memcpy(&stAQBinMM, &stAQBinDefaultValue, sizeof(ST_AQBIN_PARA));

    printf("msAPI_Initial_AQBinPara\n");

}


//Use this function need terminate screen stop vdec...use POOL buffer as temp buffer
BOOLEAN msAPI_AQ_loadBin(void)
{
    BOOLEAN bRet = FALSE;
    BININFO BinInfo;
    BOOLEAN bResult = 0;
    U8 u8i = 0;
    U8 u8MagicHeader[4]={0x87, 0x59, 0x87, 0x59};
    U8 u8NumOfWorkSheet = 0;
    U8 u8NumOfSource = (U8)AQ_SOURCE_SIZE;
    U16 u16AQWorkSheetSize = (U16)AQ_WORKSHEET_SIZE;
    U8 u8OffsetOfHeader = (U8)AQ_HEADER_OFFSET;
    U8 u8Index = 0;
    DEBUG_AQ_BIN(printf("msAPI_AQ_loadBin\n"););

    BinInfo.B_ID = BIN_ID_AQ_BIN;
    MDrv_Sys_Get_BinInfo(&BinInfo, &bResult);

    if (bResult != PASS)
    {
        printf("\nError: Fail to find AQbin bin=0x%X on flash.\n", BinInfo.B_ID);
        return FALSE;
    }

    //U32 u32Buf = ((POOL_BUFFER_ADR & MIU1) ? (POOL_BUFFER_ADR | MIU_INTERVAL) : (POOL_BUFFER_ADR)); //Temp use POOL Buffer
    //U32 u32CheckSum =0;
    ...
    if(SUCCESS == MDrv_DMA_LoadBin(&BinInfo, _PA2VA(POOL_BUFFER_ADR), _PA2VA(u32LoadBin_Dec), _PA2VA(u32LoadBin_Tmp)))
    {
        //load bin data into variable...
        //Header & End
        U8* pAQParam;
        pAQParam = PA2KSEG0(POOL_BUFFER_ADR);
        u8NumOfWorkSheet= (U8)(*pAQParam);
        DEBUG_AQ_BIN(printf("Num of Work Sheet = %d\n", u8NumOfWorkSheet););
        DEBUG_AQ_BIN(printf("Load Model 0 table as default\n"););
        u8Index = (u8Index+u8OffsetOfHeader);

        if((u8MagicHeader[0] == *(pAQParam+u8Index))&&(u8MagicHeader[1] == *(pAQParam+u8Index+1))&&(u8MagicHeader[2] == *(pAQParam+u8Index+2))&&(u8MagicHeader[3] == *(pAQParam+u8Index+3)))
        {
            DEBUG_AQ_BIN(printf("Magic number OK...is: 0x%x %x %x %x\n", *(pAQParam+u8Index), *(pAQParam+u8Index+1), *(pAQParam+u8Index+2), *(pAQParam+u8Index+3)););
            for(u8i= 0; u8i < u8NumOfSource; u8i++)
            {
                //copy source data....each source size is "AQ_WORKSHEET_SIZE" bytes
                u8Index += ((u8i*u16AQWorkSheetSize));
                if(u8i == 0)
                    memcpy(&stAQBinDTV, (pAQParam+u8Index), u16AQWorkSheetSize);
                else if(u8i == 1)
                    memcpy(&stAQBinATV, (pAQParam+u8Index), u16AQWorkSheetSize);
                else if(u8i == 2)
                    memcpy(&stAQBinHDMI, (pAQParam+u8Index), u16AQWorkSheetSize);
                else if(u8i == 3)
                    memcpy(&stAQBinPC, (pAQParam+u8Index), u16AQWorkSheetSize);
                else if(u8i == 4)
                    memcpy(&stAQBinAV, (pAQParam+u8Index), u16AQWorkSheetSize);
                else if(u8i == 5)
                    memcpy(&stAQBinMM, (pAQParam+u8Index), u16AQWorkSheetSize);
            }
        }
        else
        {
            DEBUG_AQ_BIN(printf("Magic number error...is: 0x%x %x %x %x\n", *(pAQParam+u8Index), *(pAQParam+u8Index+1), *(pAQParam+u8Index+2), *(pAQParam+u8Index+3)););
            return bRet;
        }
    ...
    {
        printf("msAPI_AQ_loadBin load bin fail!!!");
    }
...
void msAPI_SetAQBinParaIntoAudio(void)
{

    ST_AQBIN_PARA stAQBinTemp;
    U8 u8i = 0;
    U8 u8PathID =0;
    ...
    {
        u8PathID = 5;
        memcpy(&stAQBinTemp, &stAQBinMM, sizeof(ST_AQBIN_PARA));
    }
    else if(UI_INPUT_SOURCE_TYPE <= UI_INPUT_SOURCE_DTMB)
    {
        memcpy(&stAQBinTemp, &stAQBinDTV, sizeof(ST_AQBIN_PARA));
        u8PathID = 0;
    }
    else if(UI_INPUT_SOURCE_TYPE == UI_INPUT_SOURCE_ATV)
    {
        memcpy(&stAQBinTemp, &stAQBinATV, sizeof(ST_AQBIN_PARA));
        u8PathID = 1;
    }
    else if((UI_INPUT_SOURCE_TYPE >= UI_INPUT_SOURCE_HDMI)&&(UI_INPUT_SOURCE_TYPE <= UI_INPUT_SOURCE_HDMI_END))
    {
        memcpy(&stAQBinTemp, &stAQBinHDMI, sizeof(ST_AQBIN_PARA));
        u8PathID = 3;
    }
    else if(UI_INPUT_SOURCE_TYPE == UI_INPUT_SOURCE_RGB)
    {
        memcpy(&stAQBinTemp, &stAQBinPC, sizeof(ST_AQBIN_PARA));
        u8PathID = 4;
    }
    else if((UI_INPUT_SOURCE_TYPE >= UI_INPUT_SOURCE_COMPONENT)&&(UI_INPUT_SOURCE_TYPE < UI_INPUT_SOURCE_RGB))
    {
        memcpy(&stAQBinTemp, &stAQBinAV, sizeof(ST_AQBIN_PARA));
        u8PathID = 2;
    }
    else if((UI_INPUT_SOURCE_TYPE >= UI_INPUT_SOURCE_AV)&&(UI_INPUT_SOURCE_TYPE < UI_INPUT_SOURCE_DMP))
    {
        u8PathID = 2;
        memcpy(&stAQBinTemp, &stAQBinAV, sizeof(ST_AQBIN_PARA));
    }
    ...
    //set AVC
    MApi_SND_SetParam1(Sound_SET_PARAM_Avc_AT, stAQBinTemp.stAVCPara.u8AttackTime, 0);
    MApi_SND_SetParam1(Sound_SET_PARAM_Avc_RT, stAQBinTemp.stAVCPara.u8ReleaseTime, 0);
    MApi_SND_SetParam1(Sound_SET_PARAM_Avc_Threshold, stAQBinTemp.stAVCPara.u8ClippingLevel, 0);

    //Set Bass Treble
    MApi_SND_SetParam1(Sound_SET_PARAM_AbsoluteBass, stAQBinTemp.u8BassGain, 0);
    MApi_AUDIO_AbsoluteTreble(stAQBinTemp.u8TrebleGain);

    //Tone effect
    MApi_SND_ProcessEnable(Sound_ENABL_Type_Tone, stAQBinTemp.u8ToneEffectEnable);
    //Mstar Surround
    MApi_SND_ProcessEnable(Sound_ENABL_Type_Surround, stAQBinTemp.u8MStarSurroundEnable);

    //DRC
    MApi_SND_ProcessEnable(Sound_ENABL_Type_DRC, stAQBinTemp.u8DRCEnable);
    MApi_SND_SetParam1(Sound_SET_PARAM_Drc_Threshold, stAQBinTemp.u8DRCClippingLevel, AUDIO_SOUNDEFFECT_SPEAKER);

    //HPF
    MApi_SND_ProcessEnable(Sound_ENABL_Type_Hpf, stAQBinTemp.bHPFEnable);
    msAPI_AUD_SetHLPF(AUDIO_COEF_HPF, stAQBinTemp.u16HPFFrequency, stAQBinTemp.u8HPFQValue);

    //K Gain, LPF_GAIN, B Gain, A Gain, NR Threshold
    MApi_SND_SetParam1(Sound_SET_PARAM_Surround_XK, stAQBinTemp.u8KGain, 0);
    MApi_SND_SetParam1(Sound_SET_PARAM_Surround_LPFGAIN, stAQBinTemp.u8LPFGain, 0);
    MApi_SND_SetParam1(Sound_SET_PARAM_Surround_XB, stAQBinTemp.u8BGain, 0);
    MApi_SND_SetParam1(Sound_SET_PARAM_Surround_XA, stAQBinTemp.u8AGain, 0);
    MApi_SND_SetParam1(Sound_SET_PARAM_NR_Threshold, stAQBinTemp.u8NRThreshold, 0);

    //Prescale
    MApi_SND_SetParam1(Sound_SET_PARAM_PreScale, AUDIO_PATH_MAIN, stAQBinTemp.stDevicePrescale.u8MainSpeaker);
    MApi_SND_SetParam1(Sound_SET_PARAM_PreScale, AUDIO_PATH_MAIN, stAQBinTemp.stDevicePrescale.u8LineOut);
    MApi_SND_SetParam1(Sound_SET_PARAM_PreScale, AUDIO_PATH_MAIN, stAQBinTemp.stDevicePrescale.u8SifOut);
    MApi_SND_SetParam1(Sound_SET_PARAM_PreScale, AUDIO_PATH_MAIN, stAQBinTemp.stDevicePrescale.u8SpdifOut);

    //PEQ
    MApi_SND_ProcessEnable(Sound_ENABL_Type_PEQ, stAQBinTemp.stPEQ.u8Enable);
    for(u8i=0; u8i<8; u8i++)
    {
        U8 u8FcHighByte  = (U8)((stAQBinTemp.stPEQ.u16Fc[u8i]&0xFF00)>>8);
        U8 u8FcLowByte = (U8)((stAQBinTemp.stPEQ.u16Fc[u8i]&0x00FF));
        //printf("Set u8FcHighByte = 0x%x, u8FcLowByte = 0x%x\n", u8FcHighByte, u8FcLowByte);
        msAPI_AUD_SetPEQ(u8i, stAQBinTemp.stPEQ.u8Gain[u8i], u8FcHighByte, u8FcLowByte, stAQBinTemp.stPEQ.u8Qvalue[u8i]);
    }

    //Smart Sound....need check UI setting
    //MApi_SND_SetParam1(Sound_SET_PARAM_AbsoluteEQ, 0, stAQBinTemp); //120Hz
    //MApi_SND_SetParam1(Sound_SET_PARAM_AbsoluteEQ, 0, ); //500Hz
    //MApi_SND_SetParam1(Sound_SET_PARAM_AbsoluteEQ, 0, ); //1.5KHz
    //MApi_SND_SetParam1(Sound_SET_PARAM_AbsoluteEQ, 0, ); //5KHz
    //MApi_SND_SetParam1(Sound_SET_PARAM_AbsoluteEQ, 0, ); //10KHz

    //SRS
    if(stAQBinTemp.u8SRSonOff)
        MApi_AUDIO_ADVSOUND_ProcessEnable(SRS_TSHD);
    else
        MApi_AUDIO_ADVSOUND_ProcessEnable(ADV_NONE);

    MApi_AUDIO_ADVSOUND_SetParam(SRS_TSHD_SET_INPUT_GAIN, stAQBinTemp.stSRSPara.u8InputGain, 0);
    MApi_AUDIO_ADVSOUND_SetParam(SRS_TSHD_SET_OUTPUT_GAIN, stAQBinTemp.stSRSPara.u8OutputGain, 0);
    MApi_AUDIO_ADVSOUND_SetParam(SRS_TSHD_SET_DC_CONTROL, stAQBinTemp.stSRSPara.u8DCGain, 0);
    MApi_AUDIO_ADVSOUND_SetParam(SRS_TSHD_SET_TRUBASS_CONTROL, stAQBinTemp.stSRSPara.u8TrubassGain, 0);
    MApi_AUDIO_ADVSOUND_SetParam(SRS_TSHD_SET_SPEAKERSIZE, stAQBinTemp.stSRSPara.u8SpeakerSize, 0);
    MApi_AUDIO_ADVSOUND_SetParam(SRS_TSHD_SET_DEFINITION_CONTROL, stAQBinTemp.stSRSPara.u8Definition, 0);
    MApi_AUDIO_ADVSOUND_SetParam(SRS_TSHD_SET_SURROUND_LEVEL, stAQBinTemp.stSRSPara.u8SurroundLevel, 0);

    //Volume Curve
    //MApi_AUDIO_SetAbsoluteVolume( MS_U8 u8Path, MS_U8 u8Vol1, MS_U8 u8Vol2  )

}


//void msAPI_SetSwUpdateDataIntoAQBin(void)
//{
//}
