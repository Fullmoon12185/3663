...
BOOLEAN msAPI_SI_Action_CheckServiceInfo_DVB( SI_DTV_CHANNEL_INFO *pstSI_DTV_CHANNEL_INFO )
{
    ...
    if(OSD_COUNTRY_SETTING==OSD_COUNTRY_VIETNAM)
    {
         // If E_TYPE_DTV and video pid  not exists and audio pid exists,typy_radio
        if((pstSI_DTV_CHANNEL_INFO->stCHAttribute.bServiceType == E_TYPE_DTV) && ( pstSI_DTV_CHANNEL_INFO->wVideo_PID == SI_INVALID_PID ) \
            && ( pstSI_DTV_CHANNEL_INFO->stAudInfo[0].wAudPID != SI_INVALID_PID ) )
        {
            pstSI_DTV_CHANNEL_INFO->stCHAttribute.bServiceType = E_TYPE_RADIO;
        }
