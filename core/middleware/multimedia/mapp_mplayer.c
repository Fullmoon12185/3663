...
#else
#define STORE_SUBTITLE_ENTRY_TO_SUBTITLE_BUFFER     0
#endif
...
static BOOLEAN _MApp_MPlayer_Browser_EnumCurDirectory(void)
{
    ...
    else
    {
        ...
        {
        #if ENABLE_MPLAYER_SORT
            if(m_u16BufDirectory[u16Depth] != m_u16CurDirectoryPhy[u16Depth])
        #else
    ...
    if(bNeedRead)
    {
        ...
        while(pCurrentDirEntry && (u16DirCnt<NUM_OF_MAX_FILES_PER_DIRECTORY))
        {
            MApp_FileBrowser_GetCurrentFileEntry(&fileEntry);
...
{
    #define MAX_FILENAME_LENGTH_SIZE 512
    ...
    else
    {
        ...
        pu16EntryName = (U16*)msAPI_Memory_Allocate(MAX_FILENAME_LENGTH_SIZE, BUF_ID_FILEBROWER);
        ...
        // Get long file name from file entry.
        u16Len = MApp_FileBrowser_GetLongFileNameByFileEntry(
                     &stFileInfo.fileEntry,
                     (U32)(pu16EntryName),
                     (MAX_FILENAME_LENGTH_SIZE/2-1));

        u16Len = (PATH_INFO_BUFFER_LEN/2 - m_u16PathIndex[u16Index] > u16Len)?
                    u16Len:
                    PATH_INFO_BUFFER_LEN/2 - m_u16PathIndex[u16Index];

        for (u16i = 0; (u16i < u16Len) && (pu16EntryName[u16i] != 0x0U); u16i++)
        {
            pu16FullPath[u16i] = pu16EntryName[u16i];
        }

        if(m_u16PathIndex[u16Index] + u16i >= PATH_INFO_BUFFER_LEN/2)
        {
            printf("PATH_INFO_MEM is full! Directory name will be truncated or droped!! Depth = %d\n", u16Index);
            pu16FullPath[u16i-1] = '\\';
            m_u16PathIndex[u16Index + 1] = PATH_INFO_BUFFER_LEN/2;
        }
        else
        {
            if (u16i > 0)
...
#define _SHOWN_LONG_FILE_NAME_MAX_LEN        22
    ...
    else
    {
        ...
        else
        {
            ...
            {
                memcpy(&pu16Buffer[2], &pu16Src[u16Slash[u16i-1]], (_SHOWN_LONG_FILE_NAME_MAX_LEN-3)*2);
                pu16Buffer[_SHOWN_LONG_FILE_NAME_MAX_LEN-1] = 0x2026U;
                pu16Buffer[_SHOWN_LONG_FILE_NAME_MAX_LEN] = '\\';
            }
            else
...
enumMPlayerRet MApp_MPlayer_EnterDirectory(enumMPlayerFileIndexBegin eBegin, U16 u16Idx)
{
    ...
    {
        BOOLEAN bPop = FALSE;
        U16 u16BackupDirectory = m_u16CurDirectory[m_u16CurDirectoryDepth-1];
    #if ENABLE_MPLAYER_SORT
        U16 u16BackupPhyDirectory = m_u16CurDirectoryPhy[m_u16CurDirectoryDepth-1];
    #endif
        if((strncmp((char*)pCurrentDirEntry->ps8DirEntryName, "..      ", 8) == 0) &&
           (strncmp((char*)pCurrentDirEntry->ps8DirEntryExtName, "   ", 3) == 0))
        {
        ...
        if(m_u16CurDirectoryDepth <= NUM_OF_MAX_DIRECTORY_DEPTH)
        {
            ...
            if(pCurrentDirEntry->u8DirEntryAttrib & ATTRIB_DIRECTORY)
            {
                if(MApp_FileBrowser_EnterDirectory(0)==FALSE)
                {
                    //if enter directory fail, need to recovery directory path status.
                    if(bPop)
                    {
                        m_u16CurDirectory[m_u16CurDirectoryDepth] = u16BackupDirectory;
                    #if ENABLE_MPLAYER_SORT
                        m_u16CurDirectoryPhy[m_u16CurDirectoryDepth] = u16BackupPhyDirectory;
                    #endif
                        m_u16CurDirectoryDepth++;
                    }
                    else
