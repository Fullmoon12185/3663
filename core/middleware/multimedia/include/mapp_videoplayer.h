...
#define VDPLAYER_AEON_DATABUFF_SIZE VDPLAYER_DATA_LEN-VDPLAYER_SHAREMEM_SIZE
    ...
    #if(ENABLE_HK_MP4)
        // VDPLAYER_DATA size only need 1M
        #if( VDPLAYER_DATA_LEN > 0x100000 )
            #warning "VDPLAYER_DATA size only need 1M"
        #endif

    #else
        // VDPLAYER_DATA size need > 3.5M
        #if( VDPLAYER_DATA_LEN < 0x300000 )
            #error "VDPLAYER_DATA size need > 3.5M"
        #endif
    ...
    #if( VDPLAYER_DATA_LEN < 0x1000 )
        #error "VDPLAYER_DATA size need at least >= 4K for EN_VDP_SHAREMEMORY data usage"
    #endif
