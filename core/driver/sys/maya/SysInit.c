...
#define PROTECT_BEON_FULL_RANGE   1
       ...
       U8 au8MiuProtectIds[16] = {MIU_CLIENT_R2M_W, MIU_CLIENT_BDMA_RW, MIU_CLIENT_USB_UHC0_RW, MIU_CLIENT_USB_UHC1_RW, MIU_CLIENT_USB_UHC2_RW,0};
#else
...
#if 0 // Protect sec buf buffer
    {
        U8 au8ProtectIds_3[4] = {MIU_CLIENT_TSP_W, 0 };

        msAPI_MIU_Set_MiuProtect(2,
            au8ProtectIds_3, SECBUF_START_ADR,
            SECBUF_START_ADR + SECBUF_START_LEN,
            ENABLE);
    }
#endif

#if 1 // Protect the buffer after TSP_VQ to prevent TSP over access
    {
        U8 au8ProtectIds_1[4] = {0, 0 };

        msAPI_MIU_Set_MiuProtect(1,
            au8ProtectIds_1, TSP_VQ_BUFFER_CHECK_ADR,
            TSP_VQ_BUFFER_CHECK_ADR + TSP_VQ_BUFFER_CHECK_LEN,
            ENABLE);
    }
#endif

#if 1 // Protect GE buffer
    {
        U8 au8ProtectIds_3[4] = {MIU_CLIENT_R2M_W,MIU_CLIENT_GE_RW, 0 };

        msAPI_MIU_Set_MiuProtect(3,
            au8ProtectIds_3, GE_FRAMEBUFFER_ADR,
            GE_FRAMEBUFFER_ADR + GE_FRAMEBUFFER_LEN,
            ENABLE);
    }
#endif
...
/*
    {
        msAPI_MIU_WriteMemByte(0x300000, 0xAA);
        printf("1. 0x%X\n", msAPI_MIU_ReadMemByte(0x300000) );
        CHECK_MIU_PROTECT_LOG_AND_CLEAR();

        msAPI_MIU_WriteMemByte(0x300000, 0x55);
        printf("2. 0x%X\n", msAPI_MIU_ReadMemByte(0x300000) );
        CHECK_MIU_PROTECT_LOG_AND_CLEAR();

        msAPI_MIU_WriteMemByte(0x100000, 0xAA);
        printf("3. 0x%X\n", msAPI_MIU_ReadMemByte(0x100000) );
        CHECK_MIU_PROTECT_LOG_AND_CLEAR();

        msAPI_MIU_WriteMemByte(0x100000, 0x55);
        printf("4. 0x%X\n", msAPI_MIU_ReadMemByte(0x100000) );
        CHECK_MIU_PROTECT_LOG_AND_CLEAR();

        msAPI_MIU_WriteMemByte(0x8100000, 0xAA);
        printf("5. 0x%X\n", msAPI_MIU_ReadMemByte(0x100000) );
        printf("6. 0x%X\n", msAPI_MIU_ReadMemByte(0x8100000) );
        CHECK_MIU_PROTECT_LOG_AND_CLEAR();

        msAPI_MIU_WriteMemByte(0x8300000, 0x55);
        printf("7. 0x%X\n", msAPI_MIU_ReadMemByte(0x300000) );
        printf("8. 0x%X\n", msAPI_MIU_ReadMemByte(0x8300000) );
        CHECK_MIU_PROTECT_LOG_AND_CLEAR();
    }
*/
