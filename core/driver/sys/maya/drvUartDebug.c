...
#if defined(__aeon__)//def __AEONR2__
...
/******************************************************************************/
#include "drvUartDebug_Common.c"  //include common file for all chip
...
void MDrv_UART_ExecTestCommand(void)
{
    switch( g_UartCommand.Buffer[_UART_CMD_INDEX1_] )
    {
        ...
        /*********************************************************************/
        /************** Keypad Emulator of MStar TV System Tool **************/
        /*********************************************************************/
        case 0x60:
        ...
        case 0x68:
        {
            extern BOOL g_bAESKeyInitDone;
            extern void msAPI_combo_GetHDCPRevID(U8 * u8Buf);
            U8 u8TempBUF[5] = {0};
            if(g_bAESKeyInitDone)
            {
                msAPI_combo_GetHDCPRevID((U8 *)&u8TempBUF);
                printf("u8TempBUF[0] %x\n",u8TempBUF[0]);
                printf("u8TempBUF[1] %x\n",u8TempBUF[1]);
                printf("u8TempBUF[2] %x\n",u8TempBUF[2]);
                printf("u8TempBUF[3] %x\n",u8TempBUF[3]);
                printf("u8TempBUF[4] %x\n",u8TempBUF[4]);
            }
            else
            {
                printf("Key init not ready\n");
            }
            break;
        }
    #endif
    ...
    #endif // #if (ENABLE_SECURITY_R2_HDCP22)
...
//            u32MonitorOsdTimer = msAPI_Timer_GetTime0();
        ...
        case 0xDA:
        {
            ...
            MApp_Scaler_SetOutputPanelType(g_PNL_TypeSel,MAIN_WINDOW);
...
void MDrv_UART_DecodeNormalCommand(void)
{
    ...
    U8 u8Value=0;
#if (PHOENIX_DEBUG && ENABLE_DTV)
    U16 RegAddr, RegData=0;
#endif
    U16 u16EEADDR;


    if( UART_CMD != uartTest_Command )
    {
        if( g_UartCommand.u8CmdLen <= 2 )
        {
            printf("Invalid cmd!\n");
            return;
        }
    ...
    switch ( UART_CMD )
    {
        ...
        case uartWrite_MCU_XDATA:
           ...
           {
                   if(MApp_UsbDownload_USB_Update_GetPort() == SWUpdateForMFC)
                   {
                         #if(ENABLE_MFC_6M30)
...
void MDrv_UART2_DebugInit(void)
{
    ...
    u16Res = mdrv_uart_connect(E_UART_PORT3, E_UART_PIU_UART1);
