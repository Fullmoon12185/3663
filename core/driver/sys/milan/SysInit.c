...
//    U32 testvar;
    ...
    //console_init();
    //uart_init(12000000ul, 38400ul);   //for T3 FPGA test


//    printf("start here\n");
//    testvar=mfspr(SPR_SR);
//    printf("spr_sr=%08LX\n",testvar);
//    testvar=mfspr(SPR_ICBIR);
//    printf("SPR_ICBIR=%08LX\n",testvar);
    ...
    MDrv_MIU_Protect(MIU_PROTECT_IDX_0, zero_array, ALIGN(u32Start + ((1<<pageshift) - 1), pageshift), ALIGN(u32End, pageshift), ENABLE);
...
#define PROTECT_BEON_FULL_RANGE   1
    ...
    U8 au8MiuProtectIds[16] = {MIU_CLIENT_MIPS_RW, MIU_CLIENT_BDMA_RW,MIU_CLIENT_USB_UHC1_RW,MIU_CLIENT_USB_UHC2_RW, 0};
 #else
    ...
    // Set miu protect : dram_size
#if (MEMORY_MAP == MMAP_128MB)
    MDrv_WriteByteMask(0x1012D3, 0x70, 0xF0); // 128M
#else
    MDrv_WriteByteMask(0x1012D3, 0x60, 0xF0); // 64M
#endif
...
#if 0 // Test PVR buffer
    {
        U8 au8ProtectIds_3[4] = {0};

        msAPI_MIU_Set_MiuProtect(MIU_PROTECT_IDX_3,
            au8ProtectIds_3, PVR_TRICK_MODE_ADR,
            NONE_ALIGN_MHEG5_ADR,
            ENABLE);
    }
#endif
...
#if 0//_TEMP_S7L_ENABLE_URSA_
#if LOAD_BIN_TO_URSA
    MDrv_WriteRegBit(0x101EAE,0,BIT1|BIT0);//configure DDCR as GPIO
    MDrv_WriteRegBit(0x102b17,0,BIT1);//Configure PAD_GPIO23 as output
    MDrv_WriteRegBit(0x102b17,1,BIT0);//set PAD_GPIO23 as output high
    MsOS_DelayTask(1000);
    MDrv_WriteRegBit(0x102b17,0,BIT0);//set PAD_GPIO23 as output low
    MApp_MFC_LoadBinary();
#endif
#endif
