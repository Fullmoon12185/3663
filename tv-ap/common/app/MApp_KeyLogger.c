...
#define KEYLOG_FILENAME_BUF_SIZE    60
...
BOOL MApp_KeyLogger_ModifyConfigBin(EN_KEYLOGGER_MODE eMode)
{
    ...
    U8 u8HandleNo;
    U8 u8FileName[KEYLOG_FILENAME_BUF_SIZE] = {0};
    U16 u16FileName[KEYLOG_FILENAME_BUF_SIZE/2] = {0};
    ...
    //strncpy((char *)u8FileName, _KEY_LOGGER_CONFIG_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
    Str8_StrCpy_CheckDest((char *)u8FileName, _KEY_LOGGER_CONFIG_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
...
//    U32 u32DummyIdx = 0;
    ...
    *(pu8CaptureBufferAddr + _KEY_LOGGER_BIN_CRC_OFFSET_ + 1) = (U8)(u16Crc>>8);
    ...
    if( ePackMode == EN_KEYLOGGER_PACK_MODE_KEY_EVENT )
    {
        //strncpy((char *)u8FileName, _KEY_LOGGER_EVENTDB_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
        Str8_StrCpy_CheckDest((char *)u8FileName, _KEY_LOGGER_EVENTDB_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
    }
    else if (ePackMode == EN_KEYLOGGER_PACK_MODE_DATABASE)
    {
        //strncpy((char *)u8FileName, _KEY_LOGGER_DATABASE_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
        Str8_StrCpy_CheckDest((char *)u8FileName, _KEY_LOGGER_DATABASE_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
    }
    else //if (ePackMode == EN_KEYLOGGER_PACK_MODE_PARTIAL_FLASH)
    {
        //strncpy((char *)u8FileName, _KEY_LOGGER_FLASH_BKP_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
        Str8_StrCpy_CheckDest((char *)u8FileName, _KEY_LOGGER_FLASH_BKP_BIN_NAME_, KEYLOG_FILENAME_BUF_SIZE);
    }
