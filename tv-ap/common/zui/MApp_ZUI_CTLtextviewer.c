...
#define MAPP_ZUI_CTL_TEXTVIEWER_C
...
S32 MApp_ZUI_CTL_TextViewerWinProc(HWND hWnd, PMSG pMsg)
{
    switch(pMsg->message)
    {
        case MSG_PAINT:
                ...
                //if (style_list)
                {
                    //while(style_list->component != CP_NOON)
                    {
                        ...
                        //if (style_list->component == CP_TEXT_OUT)
                        {
                            ...
                            if (u16TxtComponentIndex != 0xFFFF)
                            {
                                ...
                                //if (dyna)
                                {
                                    ...
                                    u16Temp = (U16)MApp_ZUI_API_GetWindowData(hWnd);
                                    param->rect->height = param->rect->height/u16Temp;
                                    s16TopTemp = param->rect->top;
                                    dyna.max_row=1;
                                    for(i=1;i<=u16Temp;i++)
                                    {
...
#undef MAPP_ZUI_CTL_TEXTVIEWER_C
