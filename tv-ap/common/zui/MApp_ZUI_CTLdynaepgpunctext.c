...
U8 u8Scroll_BaseLine;
U8 u8Scroll_TotalCount;

U16 u16EWS_ShowStartPosition;
...
S32 MApp_ZUI_CTL_EWS_SLIDE_SCROL_WINPROC(HWND hWnd, PMSG pMsg)
{
    switch(pMsg->message)
    {
        case MSG_TIMER:
            if (u16EWS_ShowStartPosition == 0)
            {
                MApp_ZUI_API_SetTimer(hWnd, 0, 500);
                
                u16EWS_ShowStartPosition++;
            }
            else
            {
                u16EWS_ShowStartPosition++;
        ...
        case MSG_PAINT:
            {
                ...
                _MApp_ZUI_API_DefaultOnPaint(hWnd, param, FALSE);
                {
                    ...
                    if (u16TxtComponentIndex != 0xFFFF && pStr)
                    {
                       ...
                       if (u16EWS_ShowStartPosition >= MApp_ZUI_API_Strlen(pStr))
                       {
                       ...
                       else
                        {
                            ...
                            pStr = pStr+ u16EWS_ShowStartPosition;
                            dyna.pString = pStr;
                            width = msAPI_OSD_GetStrWidth(Font[dyna.eSystemFont].fHandle, (U8*)pStr, &clrBtn1);

                              // printf("444444444 u8EWS_ShowStartPosition   =%d   width=%d  param->rect->width=%d \n ",u8EWS_ShowStartPosition,width,param->rect->width);
...
S32 MApp_ZUI_CTL_EWSScrollTextWinProc(HWND hWnd, PMSG pMsg)
{
    switch(pMsg->message)
    {
        case MSG_PAINT:
                        ...
                        //if (style_list->component == CP_TEXT_OUT)
                        {
                            ...
                            if (u16TxtComponentIndex != 0xFFFF && pStr)
                            {
                                ...
                                //if (dyna)
                                {
                                    ...
                                    clrBtn1.x =  param->rect->left+2;
                                    clrBtn1.y =  param->rect->top;
                                    clrBtn1.width =  param->rect->width-8;
                                    //clrBtn1.height =  param->rect->height;
                                    clrBtn1.height = (param->rect->height)/dyna.max_row;
                                    //printf("(param->rect->height)=%d\n",(param->rect->height));
                                    //printf("dyna.max_row=%d\n",dyna.max_row);

                                    clrBtn1.t_clr = dyna.TextColor;
                                    
                                    clrBtn1.Fontfmt.flag = dyna.flag;
                                    clrBtn1.Fontfmt.ifont_gap = dyna.u8dis;
                                    clrBtn1.bStringIndexWidth = CHAR_IDX_2BYTE;
                                    _MApp_ZUI_API_DrawDynamicComponent(CP_SCROL_PUNCTUATED_DYNAMIC, &dyna, &param->dc, param->rect);
                                    //_ZUI_FREE(dyna);
                                }
                            }
                        }
...
#if TXTFONT_ZOOM2X
S32 MApp_ZUI_CTL_DynamicZoom2XPunctuatedTextWinProc(HWND hWnd, PMSG pMsg)
{
    switch(pMsg->message)
    {
        case MSG_PAINT:
                        ...
                        //if (style_list->component == CP_TEXT_OUT)
                        {
                            ...
                            if (u16TxtComponentIndex != 0xFFFF && pStr)
                            {
                                ...
                                //if (dyna)
                                {
                                    ...
                                    dyna.pString = pStr;
                                    dyna.max_row = MApp_ZUI_API_GetWindowData(hWnd);
                                    _MApp_ZUI_API_DrawDynamicComponent(CP_ZOOM2X_PUNCTUATED_DYNAMIC, &dyna, &param->dc, param->rect);
                                    //_ZUI_FREE(dyna);
                                }
                            }
                        }
