...
void MApp_Game_ProcessKey(U8 u8key)
{
    ...
    //static U8 u8GamePadStatus[GAMEPAD_KEY_NUM] = {0};
    //devGamepad_Get_KeyStatus(u8GamePadStatus, GAMEPAD_KEY_NUM);
...
#if(ENABLE_GAMEPAD)
#define GAMEPAD_TASK_PERIOD 100
U8 MApp_Game_Gamepad_Task(void)
{
    static U32 s_u32GamepadTask_LastTime = 0;
    U8 u8GamepadKey = KEY_NULL;

    if( msAPI_Timer_DiffTime_2(s_u32GamepadTask_LastTime, MsOS_GetSystemTime()) > GAMEPAD_TASK_PERIOD )
    {
        s_u32GamepadTask_LastTime = MsOS_GetSystemTime();

        devGamepad_Task();

        U8 au8KeyStatusBuf[GAMEPAD_KEY_MAX_NUM] = {0};

        // If any key pressed
        if( devGamepad_Get_KeyStatus(devGamepad_Get_FirstGamepadPort(), au8KeyStatusBuf, GAMEPAD_KEY_MAX_NUM) )
        {
            if( au8KeyStatusBuf[GAMEPAD_KEY_UP] == GAMEPAD_ISKEYPRESSED )
            {
                u8GamepadKey = KEY_UP;
            }
            else if( au8KeyStatusBuf[GAMEPAD_KEY_DOWN] == GAMEPAD_ISKEYPRESSED )
            {
                u8GamepadKey = KEY_DOWN;
            }
            else if( au8KeyStatusBuf[GAMEPAD_KEY_LEFT] == GAMEPAD_ISKEYPRESSED )
            {
                u8GamepadKey = KEY_LEFT;
            }
            else if( au8KeyStatusBuf[GAMEPAD_KEY_RIGHT] == GAMEPAD_ISKEYPRESSED )
            {
                u8GamepadKey = KEY_RIGHT;
            }
        }
    }
...
{
#if(ENABLE_GAMEPAD)
    U8 u8GamepadKey = MApp_Game_Gamepad_Task();
#endif

    MApp_ProcessUserInput();
    U8 u8IRKeyCode = u8KeyCode;
    u8KeyCode = KEY_NULL;

    U8 u8GameKey = KEY_NULL;

#if(ENABLE_GAMEPAD)
    if( u8GamepadKey != KEY_NULL )
    {
        u8GameKey = u8GamepadKey;
    }
    else
#endif
    if( u8IRKeyCode != KEY_NULL )
    {
        u8GameKey = u8IRKeyCode;
    }

    MApp_Game_ProcessKey(u8GameKey);
    ...
    if( s_pstCurGameInfo->pfGameTask )
    {
        ...
        //printf("[%u]", msAPI_Timer_DiffTime_2(u32T1, MsOS_GetSystemTime()));
    }
