...
//  printf("\r\nCurrentS2idx=%d [%d]",CurrentS2idx,TP_temp.SatelliteId);
    ...
    {
        char* pcTmpStr = "ASTRA 1H,1KR,1L,1M";
        //MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8 *)pcTmpStr, strlen(pcTmpStr));
        MApp_U8StrToU16Str_CheckBufSize( (U8*)pcTmpStr, (U16*)CHAR_BUFFER, STRING_BUFFER_LENGTH);
    }
    else
    {
        //MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8 *)sat.aSatName, strlen(sat.aSatName) );
        MApp_U8StrToU16Str_CheckBufSize( (U8*)sat.aSatName, (U16*)CHAR_BUFFER, STRING_BUFFER_LENGTH);
    }
...
LPTSTR _GetChInfoFreqSymbolTxt(void)
{
    ...
    WORD CurrentS2idx = msAPI_CM_GetPhysicalidx(bServiceType, wCurrentPosition);

    DTVPROGRAMID_M pCarrierParam;

    if( GetIDTable(CurrentS2idx, (BYTE *)&pCarrierParam, E_DATA_ID_TABLE) )
    {
    #define S2_STR_FREQ_SYMBOL_BUF_SIZE 20
        char au8TmpStrBuf[S2_STR_FREQ_SYMBOL_BUF_SIZE];

        if( pCarrierParam.u8Polarity )
            snprintf( au8TmpStrBuf, S2_STR_FREQ_SYMBOL_BUF_SIZE, "%d V %d ",pCarrierParam.u32S2Frequency, pCarrierParam.u16SymbolRate);
        else
            snprintf( au8TmpStrBuf, S2_STR_FREQ_SYMBOL_BUF_SIZE, "%d H %d ",pCarrierParam.u32S2Frequency, pCarrierParam.u16SymbolRate);

        //MApp_ZUI_API_StringBuffU8toU16(CHAR_BUFFER, (U8 *)au8TmpStrBuf, strlen(au8TmpStrBuf));
        MApp_U8StrToU16Str_CheckBufSize( (U8*)au8TmpStrBuf, (U16*)CHAR_BUFFER, STRING_BUFFER_LENGTH);
        return CHAR_BUFFER;
    }
    else
    {
...
{
#define S2_STR_SIGNAL_QUALITY_BUF_SIZE 20

    char au8TmpStrBuf[S2_STR_SIGNAL_QUALITY_BUF_SIZE];

    snprintf( (char*)au8TmpStrBuf, S2_STR_SIGNAL_QUALITY_BUF_SIZE, "Quality: %d", msAPI_Tuner_GetSignalQualityPercentage() );
