...
U8* AMPMText(U8 u8Hour)
{
    ...
    else // 12-hour notation
    {
        if (u8Hour < 12)
         ...
         MApp_U16StrToU8Str_CheckBufSize(MApp_ZUI_API_GetString(u16TempStringID), (U8*)CHAR_BUFFER, STRING_BUFFER_LENGTH );
...
{
#if 1
    snprintf((char*)CHAR_BUFFER, STRING_BUFFER_LENGTH, "%dx%d", gstVidStatus.u16HorSize, gstVidStatus.u16VerSize);
#else

    U8 tempCount;
    tempCount=MApp_GetNoOfDigit(gstVidStatus.u16HorSize);
    MApp_UlongToU8String(gstVidStatus.u16HorSize, (U8*)CHAR_BUFFER, tempCount);
    ((U8*)CHAR_BUFFER)[tempCount++]='x';
    MApp_UlongToU8String(gstVidStatus.u16VerSize, ((U8*)CHAR_BUFFER)+tempCount, MApp_GetNoOfDigit(gstVidStatus.u16VerSize));
#endif
...
void UiMenuStr_GetVChip2RatingString(U8 *pu8Str, U16 u16StrBufSize)
{
    ...
    // todo: Use u16StrBufSize to check ...
    UNUSED(u16StrBufSize);
...
static void UiMenuStr_GetRatingString(U8 *pu8Str, U16 u16StrBufSize)
{
    U8 u8Strlen;
    BOOLEAN bExtra = TRUE;
    ...
    if (g_stVChipRatingInfo.RatingType == VCHIP_RATING_TYPE_REG5)
    {
        if (bExtra)
        {
            u8Strlen = UiMenuStr_GetVChip1RatingString(pu8Str, u16StrBufSize);
            pu8Str += u8Strlen;
        #if _VChip2_
    ...
    #if _VChip2_
        if( u16StrBufSize > u8Strlen )
        {
            UiMenuStr_GetVChip2RatingString(pu8Str, u16StrBufSize - u8Strlen);
        }
    #endif
    }
...
{
#define RATING_INFO_TEXT_LEN    256

    memset(au8Section, 0, RATING_INFO_TEXT_LEN);

    UiMenuStr_GetRatingString(au8Section, RATING_INFO_TEXT_LEN);
...
U8* CurEPGProgramInfoText(void)
{
    ...
    if(IsDTVInUse())  // Remove ATV case.....ATV no EPG ui
    {
        ...
        //strncpy( (char*)au8Section, (char*)u8EpgTitlePrefix, SYS_SECTION_BUF_SIZE );
        Str8_StrCpy_CheckDest( (char*)au8Section, (char*)u8EpgTitlePrefix, SYS_SECTION_BUF_SIZE );
        ...
        if( u8StrBuffer2 != NULL )
        {
            ...
            //au8Section[i] = NULL;
            //strncpy( (char*)au8Section+i, (char*)u8StrBuffer2, SYS_SECTION_BUF_SIZE-i);
            Str8_StrCpy_CheckDest( (char*)au8Section+i, (char*)u8StrBuffer2, SYS_SECTION_BUF_SIZE-i);
...
U8* EventInfoLine2Left(void)
{
    ...
    if(IsDTVInUse())
    {
        u8StrLen = strlen((char *)DtvReslutionTypeText());
        memcpy(au8Section, DtvReslutionTypeText(), (u8StrLen)); //including '\0'
        u8BufLen += u8StrLen;
    }
    else
    {
        MApp_ReturnResolution(au8Section, SYS_SECTION_BUF_SIZE);
        u8StrLen = strlen((char *)au8Section);
        u8BufLen += u8StrLen;
    }

    au8Section[u8BufLen++] = CHAR_SPACE;
    au8Section[u8BufLen++] = CHAR_SPACE;
    ...
    if((pstVirtualCh->u8AudLangNum && IsDTVInUse()) || IsATVInUse())
    {
        if(IsDTVInUse())
        {
            u8StrLen = strlen((char *)AudioTitleText());
            memcpy(au8Section+u8BufLen, (AudioTitleText()), u8StrLen);
            u8BufLen +=u8StrLen;

            au8Section[u8BufLen++] = CHAR_SPACE;
            MApp_UlongToString(g_u8AudLangSelected + 1, au8Section+u8BufLen, 1);
            u8BufLen++;

            au8Section[u8BufLen++] = CHAR_SLASH;
            MApp_UlongToString(pstVirtualCh->u8AudLangNum, au8Section+u8BufLen, 1);
            u8BufLen++;

            au8Section[u8BufLen++] = '(';
            u8StrLen = strlen((char *)AudSoundModeText());
            strncpy((char *)au8Section+u8BufLen, (char *)AudSoundModeText(),u8StrLen);
            u8BufLen += u8StrLen;
            au8Section[u8BufLen++] = ')';
        }
        else if(IsATVInUse()) // Here Signal Info should be show.
        {
            if(MApp_SoundMtsSifSoundModeTrans(g_u8AudLangSelected)==E_AUDIOMODE_K_STEREO)
            {
                ...
                {
                    u8StrLen = strlen((char *)MtsStereoText());
                    memcpy(au8Section+u8BufLen, MtsStereoText(), u8StrLen);
                    u8BufLen +=u8StrLen;
                }
            }
            else if(MApp_SoundMtsSifSoundModeTrans(g_u8AudLangSelected)==E_AUDIOMODE_MONO_SAP)
            {
                ...
                {
                    u8StrLen = strlen("SAP");
                    memcpy(au8Section+u8BufLen, "SAP", u8StrLen);
                    u8BufLen +=u8StrLen;
                }
            }
            else if(MApp_SoundMtsSifSoundModeTrans(g_u8AudLangSelected)==E_AUDIOMODE_DUAL_AB)
            {
                u8StrLen = strlen("DUAL");
                memcpy(au8Section+u8BufLen, "DUAL", u8StrLen);
                u8BufLen +=u8StrLen;

            }
            else
            {
                u8StrLen = strlen("Mono");
                memcpy(au8Section+u8BufLen, "Mono", u8StrLen);
                u8BufLen +=u8StrLen;
            }
        }
    }

    au8Section[u8BufLen] = 0;
...
{
    au8Section[0] = 0;
    memset(au8Section, 0x00, SYS_SECTION_BUF_SIZE);
...
/* Scramble text*/
    ...
    if(IsDTVInUse())
    {
        u8StrLen = strlen((char *)ScrambleText());
        memcpy(au8Section+u8StrOffset, ScrambleText(), u8StrLen);
        u8StrOffset += u8StrLen;
        au8Section[u8StrOffset++] = CHAR_SPACE;
    }
  #endif

/* Rating String */
    UiMenuStr_GetRatingString(au8Section+u8StrOffset, SYS_SECTION_BUF_SIZE-u8StrOffset);

    u8StrLen = strlen((char *)au8Section);
    printf("u8StrLen = %d, rating = %s\n", u8StrLen, au8Section);
    au8Section[u8StrLen] = 0;
...
#define SIZE_OF_LOC_BUF 1024
    ...
    if (stGenSetting.g_SysSetting.fTimeAutoSync == TIME_AUTO_SYNC_OFF)
    {
        ...
        else
        {
            iu8Buffer_i += snprintf((char *)locbuff,SIZE_OF_LOC_BUF, "%04u/%02u/%02u %02u:%02u %s",
                stDate.u16Year, stDate.u8Month, stDate.u8Day, u8Hour, stDate.u8Min, AMPMText(stDate.u8Hour));
        }
    }
    else
    {
        ...
        if (IsDTVInUse() && MApp_EpgDB_GetCurEvent(&stEpgEventInfo))
        {
            ...
            if (stGenSetting.g_SysSetting.fTimeFormat == 1) // 12-hour
            {
                ...
                iu8Buffer_i += snprintf((char *)locbuff+iu8Buffer_i, SIZE_OF_LOC_BUF-iu8Buffer_i, "%02u:%02u%s-",
                        u8Hour, stDate.u8Min, AMPMText(stDate.u8Hour));
    ...
    Str8_StrCpy_CheckDest((char *)au8Section, (char *)locbuff, SYS_SECTION_BUF_SIZE);
