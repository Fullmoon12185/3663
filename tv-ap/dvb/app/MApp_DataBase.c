...
MS_BOOL MApp_memcmp(U8 *buf1, U8 *buf2, U16 size)
{
    U16 i;
    MS_BOOL ret;
...
extern U8 u8EnterUSBUpdateBinFileMode;
#define TRACE_FLASH_WRITING 1
#define FLASH_VERIFY_SIZE (0x10)

#if(FLASH_SIZE == FLASH_SIZE_8MB)
#define FLASH_4M_BLOCK_NUM (0x40)
#endif
MS_BOOL MDrv_FLASH_Write_Check(MS_U32 u32FlashAddr,MS_U32 u32FlashSize,MS_U8 * user_buffer)
{
    MS_BOOL ret;
    MS_BOOL ret2;
    U16 u16StartBank;
    ...
    u16StartBank=(u32FlashAddr>>16);
    u16EndBank=((u32FlashAddr+u32FlashSize-1)>>16);


#if(FLASH_SIZE == FLASH_SIZE_8MB)
    if((u16StartBank<(0x37+FLASH_4M_BLOCK_NUM)) && (u8EnterUSBUpdateBinFileMode == FALSE)) // code area
#elif(FLASH_SIZE == FLASH_SIZE_4MB)
    if((u16StartBank<0x37) && (u8EnterUSBUpdateBinFileMode == FALSE)) // code area
#endif
    {
        //u32SystemFailFlag|=SYSTEM_FAIL_CODEAREA_WRITE;
        printf("SYSTEM_FAIL_CODEAREA_WRITE", __FUNCTION__),
        ret=FALSE;
    }
#if(FLASH_SIZE == FLASH_SIZE_8MB)
    else if(u16StartBank>=(0x40+FLASH_4M_BLOCK_NUM))  // over flash size
#elif(FLASH_SIZE == FLASH_SIZE_4MB)
    else if(u16StartBank>=0x40)  // over flash size
#endif
    {
        //u32SystemFailFlag|=SYSTEM_FAIL_OVER_FLASH_SIZE;
        printf("SYSTEM_FAIL_OVER_FLASH_SIZE", __FUNCTION__),
        ret=FALSE;
    }
    else
    {
        if(u16StartBank!=u16EndBank) // overbank access  ,#18929 Check overbank access to prevent crashing data and help debug
        {
            //u32SystemFailFlag|=SYSTEM_FAIL_OVERBANK_ACCESS;
            printf("SYSTEM_FAIL_OVERBANK_ACCESS", __FUNCTION__),
            ret=FALSE;
        }
        else
        {
            U8 buf[FLASH_VERIFY_SIZE];
            U8 buf2[FLASH_VERIFY_SIZE];
            ret=MDrv_FLASH_Write( u32FlashAddr, u32FlashSize, user_buffer);
            if(ret)
            {
                // verify flash(both top & button)
                if(u32FlashSize>FLASH_VERIFY_SIZE)
                {
                    ret=MDrv_FLASH_Read(u32FlashAddr,FLASH_VERIFY_SIZE, buf);
                    ret2=MDrv_FLASH_Read(u32FlashAddr+u32FlashSize-1-FLASH_VERIFY_SIZE,FLASH_VERIFY_SIZE, buf2);

                    if(FALSE==ret || FALSE==ret2)
                    {
                        //u32SystemFailFlag|=SYSTEM_FAIL_FLASH_READ_FAIL;
                        printf("SYSTEM_FAIL_FLASH_READ_FAIL", __FUNCTION__);
                    }
                    else
                    {
                        ret=MApp_memcmp(&user_buffer[0], buf, FLASH_VERIFY_SIZE);
                        ret2=MApp_memcmp(&user_buffer[u32FlashSize-1-FLASH_VERIFY_SIZE], buf2, FLASH_VERIFY_SIZE);
                        if(FALSE==ret || FALSE==ret2)
                        {
                            //u32SystemFailFlag|=SYSTEM_FAIL_VERIFY_FAIL;
                            printf("SYSTEM_FAIL_VERIFY_FAIL", __FUNCTION__);
                        }
                    }
                }
                else  //                 if(u32FlashSize>FLASH_VERIFY_SIZE)
                {
                    ret=MDrv_FLASH_Read(u32FlashAddr,FLASH_VERIFY_SIZE, buf);
                    ...
                    {
                        //u32SystemFailFlag|=SYSTEM_FAIL_FLASH_READ_FAIL;
                        printf("SYSTEM_FAIL_FLASH_READ_FAIL 02", __FUNCTION__);
                    }
                    else
                    {
                        ret=MApp_memcmp(&user_buffer[0], buf, u32FlashSize);
                        if(FALSE==ret)
                        {
                            //u32SystemFailFlag|=SYSTEM_FAIL_VERIFY_FAIL;
                            printf("SYSTEM_FAIL_VERIFY_FAIL 02", __FUNCTION__);
                        }
                    }
                }
            }
            ...
            {
                //u32SystemFailFlag|=SYSTEM_FAIL_FLASH_WRITE_FAIL;
                printf("SYSTEM_FAIL_FLASH_WRITE_FAIL", __FUNCTION__);
            }
