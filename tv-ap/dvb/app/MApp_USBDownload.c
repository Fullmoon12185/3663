...
#define DOWNLOAD_BUFFER_TYPE_USB  MIU_SDRAM2SDRAM
    ...
    printf("MApp_UsbDownload_Init(Port=0x%X)\n", u8PortNo);
...
#if ((ENABLE_USB_DOWNLOAD_HDCPKEY22_BIN)||(ENABLE_USB_DOWNLOAD_BIN))
...
{
    U8 u8UsbPortNo, i, u8FileNum;
    char *acBinFilename[] = {"AQParam.bin"};

    u8FileNum = sizeof(acBinFilename) /sizeof(acBinFilename[0]);
    U16 u16tmpBinId[u8FileNum];
...
#define USB_DOWNLOAD_BIN_BUF_SIZE   0x200000
 ...
 // Check download buffer
    ...
    for(i = 0 ; i < u8FileNum; i++)
    {
        ...
        if( i ==1 && bUpgradeTextBin == FALSE)
        {
            printf("\n Error: USB upgrade %s fail !Need to upgrade %s first.\n", acBinFilename[i], acBinFilename[i-1]);
            continue;
        }

        usbDlErrCode = MW_Usb_Download_Bin(u8UsbPortNo, (U8*)acBinFilename[i], u32DownloadBufferAddr,
                                           u32DownloadBufferSize, u16tmpBinId[i]);

        if (usbDlErrCode == E_USB_DL__OK)
        {
            printf("\n ^^ . USB upgrade %s OK. Code=%u \n", acBinFilename[i], usbDlErrCode);
        }
        else
        {
            printf("\n Error: USB upgrade %s fail ! ErrCode=%u \n", acBinFilename[i], usbDlErrCode);
            if(i ==0)
            {
...
//=================================================
    ...
    u8CurFileBrowserHandle = MApp_FileBrowser_Create(0); //u8ConnectDrive����Ҫ�B��USB drive index
    ...
    MApp_USB_ChangeToSpecificDirectory((U16 *)u16SourceDir,HDCP22_FOLDER_NAME_LEN);// -> ?�}�uChakra3 _MApp_MPlayer_ChangeToSpecificDirectory�K���и���
