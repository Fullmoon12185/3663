...
#endif
#if ENABLE_SBTVD_DIRECT_TUNE
BOOLEAN bDirectTune_Status = FALSE;
#endif
...
extern MEMBER_SERVICETYPE msAPI_CHPROC_ORD_Get_ServiceType(U16 u16Ordinal);
#endif
//
//******************************************************************************
// Macro
...
/******************************************************************************/
#if ENABLE_SBTVD_DIRECT_TUNE
void MApp_SBTVD_DirectTune(U8 u8ChMajorNum) //digital key direct tune
{
 if (msAPI_ATV_GetCurrentAntenna()==ANT_CATV)
 {
    ...
    stGenSetting.stScanMenuSetting.u8ScanType = SCAN_TYPE_MANUAL;
    stGenSetting.stScanMenuSetting.u8ATVManScanType=ATV_MAN_SCAN_TYPE_CHNUM;
    stGenSetting.stScanMenuSetting.u8ATVManScanDir=ATV_MAN_SCAN_DOWN;
    enTVRetVal = EXIT_GOTO_CATVSCAN;

    bDirectTune_Status = TRUE;
 ...
 else
 {
    ...
    stGenSetting.stScanMenuSetting.u8ScanType=SCAN_TYPE_MANUAL;
    stGenSetting.stScanMenuSetting.u8RFChannelNumber = u8ChMajorNum;
    u32NumChannChangeTimer = 0;

    enTVRetVal = EXIT_GOTO_DTVSCAN;
    u16ScanDtvChNum = 0;
    u16ScanRadioChNum = 0;
#if NORDIG_FUNC //for Nordig Spec v2.0
    u16ScanDataChNum = 0;
        ...
        case KEY_SELECT:
            ...
            if (MApp_ZUI_GetActiveOSD() == E_OSD_TENKEY_NUMBER)
            {
                ...
                if (u16Ordinal != INVALID_ORDINAL)
                {
                  ...
                  if (msAPI_ATV_GetCurrentAntenna()==ANT_CATV)
                  {
                      if(msAPI_ATV_IsProgramSearched(msAPI_ATV_GetCurrentProgramNumber())==FALSE)
                      {
                        MApp_SBTVD_DirectTune(u8IdleMajorValue);
                        u8KeyCode = KEY_NULL;
                        MApp_ZUI_ACT_Startup_ChannelInfo_OSD();
                        MApp_ZUI_ACT_ExecuteWndAction(EN_EXE_SHOW_BRIEF_CH_INFO);
                        break;
                  ...
                  if(msAPI_ATV_GetCurrentAntenna()==ANT_AIR)
                  {
                   if(msAPI_CHPROC_ORD_Get_ServiceType(u16Ordinal)==E_SERVICETYPE_ATV)
                   {
                    if(msAPI_ATV_IsProgramSearched(msAPI_ATV_GetCurrentProgramNumber())==FALSE)
                    {
                      MApp_SBTVD_DirectTune(u8IdleMajorValue);
...
BOOLEAN MApp_TV_CheckScreenAudioOnly(void)
{
    if(E_OSD_SCREEN_SAVER == MApp_ZUI_GetActiveOSD() && SYS_SCREEN_SAVER_TYPE(MAIN_WINDOW) == EN_SCREENSAVER_AUDIOONLY)
        return TRUE;
    else
        return FALSE;
}

void MApp_TV_CloseOSD(void)
{
    MApp_ZUI_ACT_ShutdownOSD();
}
