...
#if ENABLE_SBTVD_DIRECT_TUNE
                else if (stGenSetting.stScanMenuSetting.u8ATVManScanType == ATV_MAN_SCAN_TYPE_CHNUM)
                {
                    msAPI_Tuner_TuningProcessor(AFT_EXT_STEP_SEARCHONECH);
                    enATVScanState = STATE_ATV_SCAN_WAIT;
                }
#endif
                else
        ...
        case STATE_ATV_SCAN_WAIT:
            ...
            else
            {
                ...
                else if(stGenSetting.stScanMenuSetting.u8ATVManScanType == ATV_MAN_SCAN_TYPE_CHNUM)
                {
                    ...
                    {
                        ATV_SCAN_DBINFO( printf( "ATV_MAN_SCAN_TYPE_CHNUM finished>>\n" ) );
        ...
        case STATE_ATV_SCAN_SHOW_INFO:
            ...
            else
            {
               ...
               if(msAPI_ATV_GetDirectTuneFlag() == FALSE)
                ...
                {
                    #if ENABLE_SBTVD_DIRECT_TUNE
                     if (MApp_ZUI_GetActive_ChannelInfo_OSD()&& bDirectTune_Status==TRUE)
                     {
