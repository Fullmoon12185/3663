...
#define ENABLE_CHECK_XC_SRC_WIN 1

#if(ENABLE_CHECK_XC_SRC_WIN)
#define XC_SRC_WIN_WIDTH_MAX    4096
#define XC_SRC_WIN_WIDTH_MIN    200

#define XC_SRC_WIN_HIGH_MAX     2160
#define XC_SRC_WIN_HIGH_MIN     100
#endif
                                            ...
                                            SCALER_WIN eWindow)
{
    ...
    // Check enInputSourceType
    if( INPUT_SOURCE_NONE == enInputSourceType )
    {
        return ;
    }
...
#endif
    {
        ...
        if (ptSrcWin) // Use the capture window from para
        {
    ...
    #if(ENABLE_CHECK_XC_SRC_WIN)
        if( (stXC_SetWin_Info[eWindow].stCapWin.width > XC_SRC_WIN_WIDTH_MAX)
          ||(stXC_SetWin_Info[eWindow].stCapWin.width < XC_SRC_WIN_WIDTH_MIN)
          )
        {
            printf("\nError: Invalid src win width=%d\n", stXC_SetWin_Info[eWindow].stCapWin.width);
            return;
        }

        if( (stXC_SetWin_Info[eWindow].stCapWin.height > XC_SRC_WIN_HIGH_MAX)
          ||(stXC_SetWin_Info[eWindow].stCapWin.height < XC_SRC_WIN_HIGH_MIN)
          )
        {
            printf("\nError: Invalid src win height=%d\n", stXC_SetWin_Info[eWindow].stCapWin.height);
            return;
        }
    #endif
...
//            u8_AR_NUM = E_AR_AUTO;
    ...
    DEBUG_ASPECT_RATIO(printf("Scaler aspect ratio remap: %u -> %u \n", enVideoScreen, u8_AR_NUM));
    return u8_AR_NUM;
}
...
// Log time for ChangeChannel
...

    {
        if( u8Enable ) // Disable BlueScreen
        {
            ...
            {
                printf("Unmute need to wait fpll done\n");
                msAPI_Scaler_SetScreenMute(E_SCREEN_MUTE_BY_SCALER_FPLL, ENABLE, 0, MAIN_WINDOW);
            }
        }
