...
U16 MApp_U8StrToU16Str_CheckBufSize( U8 *pu8SrcStr, U16 *pu16DestStr, U16 u16DestBufSize)
{
    U16 u16CopyCount = 0;

	if( (pu8SrcStr == NULL) || (pu16DestStr == NULL) )
	{
		return 0;
	}
	...
	{
    	pu16DestStr[u16CopyCount] = pu8SrcStr[u16CopyCount];
        if( pu16DestStr[u16CopyCount] == 0 )
        {
            bNullCharFound = TRUE;
            break;
        }
    ...
    if( bNullCharFound == FALSE )
    {
        ...
        pu16DestStr[u16DestBufSize-1] = 0;
    }

    return u16CopyCount;
}

U16 MApp_U16StrToU8Str_CheckBufSize( U16 *pu16SrcStr, U8 *pu8DestStr, U16 u16DestBufSize)
{
    U16 u16CopyCount = 0;

	if( (pu16SrcStr == NULL) || (pu8DestStr == NULL) )
	{
		return 0;
	}
	...
	for( u16CopyCount = 0 ;u16CopyCount < (u16DestBufSize-1); u16CopyCount += 1)
	{
    	pu8DestStr[u16CopyCount] = pu16SrcStr[u16CopyCount];
        if( pu8DestStr[u16CopyCount] == 0 )
        {
            bNullCharFound = TRUE;
            break;
        }
