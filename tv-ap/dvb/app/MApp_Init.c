...
#if ((ENABLE_DVB_T2) && ((FRONTEND_DEMOD_DVBT_TYPE == MSTAR_MSB124X_DVBT)||(FRONTEND_DEMOD_DVBT_TYPE == SI2164B_DVBT)))
...
void MApp_PreInit_PowerOnMusic_Init(void)
{
    ...
    {
    #if ENABLE_EPGTIMER_RECORDER_TURNOFFPANEL
        if ((MApp_Init_CheckOnTimeSetting()==FALSE)&&(msAPI_Power_IswakeupsourceRTC()==TRUE))
        {
...
void MApp_PreInit_Display_Step2_Init(void)
{
...
#endif
    {
        ...
        if(msAPI_AQ_loadBin() == FALSE)
            printf("Load AQ bin Fail!!!!!!!!!\n");
        msAPI_SetAQBinParaIntoAudio();
#endif
        MApp_Audio_Set_AmpMute(AUDIO_AMP_MUTE_OFF);
    }
