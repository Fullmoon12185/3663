...
#if ENABLE_CI_PLUS_V1_4
void MApp_CI_GetFreeLCN(U16 u16Lcn, U16 *u16MaxLcn, U16 *u16MinLcn, BOOL bReset)
{
    ...
    for ( u8ServiceTypeIdx = 0; u8ServiceTypeIdx < 3; u8ServiceTypeIdx += 1 )
    {
        ...
        if ( u16ProgCount > 0 )
        {
            ...
            do
            {
                ...
                MApp_CI_GetFreeLCN(stChannelInfo.wLCN, &u16MaxLcn, &u16MinLcn, FALSE);
                if (wPosition > 0)
                {
                    wPosition--;
                }
            ...
            printf("[CI+][OP][LCN Conflic] Current LCN Max=%u, Min=%u \n", u16MaxLcn, u16MinLcn);
            wPosition = u16ProgCount - 1;
            do
            {
                msAPI_CM_GetProgramInfo(eServiceType, wPosition, &stChannelInfo);
                ...
                {
                    stOpLCNConflict[u8LcnConflictIndex].enChannelIssueType = EN_OP_CHANNEL_ISSUE_TYPE_LCN_COLLISION;
                    stOpLCNConflict[u8LcnConflictIndex].u16LCN = stChannelInfo.wLCN;
                    stOpLCNConflict[u8LcnConflictIndex].u8ServiceNameLen = (U8)strlen((char*)stChannelInfo.bChannelName);
                    memcpy(stOpLCNConflict[u8LcnConflictIndex].u8aServiceName, stChannelInfo.bChannelName, stOpLCNConflict[u8ReturnValue].u8ServiceNameLen);
                    stOpLCNConflict[u8LcnConflictIndex].u8FreeLcnNumber = 2;
                    printf("[CI+][OP][LCN Conflic] LCN=%u, Name=%s \n", stChannelInfo.wLCN, stChannelInfo.bChannelName);
                    u8ReturnValue++;

                    // Min LCN
                    if (u16MinLcn - u8ReturnValue >= 1)
                    {
                        stOpLCNConflict[u8LcnConflictIndex].u16aFreeLCN[0] = u16MinLcn - u8ReturnValue;
                    }
                    else
                    {
                        stOpLCNConflict[u8LcnConflictIndex].u16aFreeLCN[0] = u16MaxLcn + u8ReturnValue;
                    }

                    // Max LCN
                    if (u16MinLcn - u8ReturnValue >= 1)
                    {
                        stOpLCNConflict[u8LcnConflictIndex].u16aFreeLCN[1] = u16MaxLcn + u8ReturnValue;
                    }
                    else
                    {
                        stOpLCNConflict[u8LcnConflictIndex].u16aFreeLCN[1] = u16MaxLcn + u8ReturnValue + 1;
                    }
                    u8LcnConflictIndex++;
                }
            ...
            printf("[CI+][OP][LCN Conflic] Count=%u \n", u8ReturnValue);
        }
    }

    MApp_CI_GetFreeLCN(0, &u16MaxLcn, &u16MinLcn, TRUE);
    printf("[%s:%d] Exit \r\n", __PRETTY_FUNCTION__, __LINE__);
...
//#endif
    ...
    {
        printf("[CI+][OP] sendOPNitManagement with EN_OP_NIT_ERROR_FIELD_NO_ISSUE (LCN COLLISION) \r\n");
...
void MApp_CI_CiEventCommandOpNit( void )
{
    ...
    if(TRUE == msAPI_CI_OP_GetNIT(pu8OP_Nit, &u16OP_NitLen))
    {
        ...
        {
            printf("[CI+][OP][NIT] Send operator_entitlement_ack()!\r\n");
    ...
    if (TRUE == msAPI_CI_OP_GetStatus(&stCiOpSearchStatus))
    {
        ...
        */
        ...
        {   // initialised_flag = 1
            ...
            {
                printf("[CI+][OP][SearchStatus] Send operator_entitlement_ack()!\r\n");
                msAPI_CI_OP_SendEntitlementAck();
            }
        }
    }
        ...
        */
        ...
        {   // initialised_flag = 1
            ...
            if ((EN_OP_REFRESH_REQUEST_FLAG_DEFAULT == stCiOpStatus.u8RefreshRequestFlag) && (FALSE == bOPNitReqSent))
            {
                if (TRUE == g_bOpEntitlementChange)
                {
                    printf("[CI+][OP][Handle] Send operator_entitlement_ack()!\r\n");
                    msAPI_CI_OP_SendEntitlementAck();
                }
                else
                {
                    ...
                    if(TRUE == MApp_GetActiveStandbyMode() && (FALSE == g_bOpEntitlementChange))
                    {
                        printf("[%s:%d][CI+][OP] Back To Standby mode \r\n", __FILE__, __LINE__);
                        MApp_Standby_Init();
                    }
                }
            }
...
BOOL MApp_CI_SendCiOpSearchStart(void)
{
    ...
    */
    ...
    if (stCiOpStatus.u8DeliverySystemHint & EN_OP_DELIVERY_SYSTEM_HINT_DVB_S)
    {
        printf("[CI+][OP] Operator Status::delivery_system_hint is DVB-S(0x43) and/or DVB-S2(0x79)\n");
#if (ENABLE_DVBS)
        u8aDeliveryCapability[u8DeliveryCapabilityIndex++] = TAG_SDSD;
        bDeliverySystemSupported = TRUE;
#else
        printf("[%s:%d][Warning!][CI+][OP] Host can't support Operator Status::delivery_system_hint DVB-S(0x43)!\r\n", __FILE__, __LINE__);
#endif

#if (ENABLE_S2)
        u8aDeliveryCapability[u8DeliveryCapabilityIndex++] = TAG_S2DSD;
        bDeliverySystemSupported = TRUE;
#else
        printf("[%s:%d][Warning!][CI+][OP] Host can't support Operator Status::delivery_system_hint DVB-S2(0x79)!\r\n", __FILE__, __LINE__);
#endif
    }
    else
    {
    ...
    if (stCiOpStatus.u8DeliverySystemHint & EN_OP_DELIVERY_SYSTEM_HINT_DVB_T)
    {
        ...
        bDeliverySystemSupported = TRUE;
#else
        printf("[%s:%d][Warning!][CI+][OP] Host can't support Operator Status::delivery_system_hint DVB-T(0x5A)!\r\n", __FILE__, __LINE__);
#endif
        ...
        bDeliverySystemSupported = TRUE;
#else
        printf("[%s:%d][Warning!][CI+][OP] Host can't support Operator Status::delivery_system_hint DVB-T2(0x7F04)!\r\n", __FILE__, __LINE__);
#endif
    }
    else
    {
