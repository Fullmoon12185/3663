...
void MApp_FuncExec_AudioVolume(FUNC_AUDIO_VOLUME_TYPE func, U8 *para1)
{
    switch(func)
    {
        case EN_FUNC_AUDIO_VOLUME_ADJ_VOLUME:
        {
            MApp_UiMenu_ARCDeviceStatusWin_Hide();
            MApp_Ui_AudioVolumePage_Notify(EN_AUDIO_VOLUME_PAGE_NOTIFY_SHOW_HIDE_ARC_DISCONNECT,FALSE);


        #if(ENABLE_CEC)
            if( MApp_CEC_ARC_Is_ExtAudioDeviceConnected() )
          /*#if(CEC_VERSION_USING == CEC_NEW_VERSION)
            if (msAPI_CEC_IsAudioSystemKeyValid() == FALSE)
          #else
            if (msAPI_CEC_ARC_Is_Connect() == FALSE)
          #endif */
            {
                MApp_Ui_AudioVolumePage_Notify(EN_AUDIO_VOLUME_PAGE_NOTIFY_SHOW_HIDE_POWER_AMPLIFIER,TRUE);
                MApp_Ui_AudioVolumePage_Notify(EN_AUDIO_VOLUME_PAGE_NOTIFY_VOLUME,FALSE);
                return ;    //to contorl amp, don't affect to local volume
            }
            else
        #endif
            {
                MApp_Ui_AudioVolumePage_Notify(EN_AUDIO_VOLUME_PAGE_NOTIFY_SHOW_HIDE_POWER_AMPLIFIER,FALSE);
            }
        ...
        case EN_FUNC_AUDIO_VOLUME_GET_VOLUME:
          ...
          #endif */
            {
                *para1 = msAPI_CEC_ARC_GetARCAudioVolume(); // anvi22
            }
            else
        #endif
            {
                *para1 = stGenSetting.g_SoundSetting.Volume ;
            }
            break;
...
void MApp_FuncExec_AdjustVolume(VIRTUAL_KEY_CODE key)
{
