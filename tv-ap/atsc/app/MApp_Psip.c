...
static void MApp_Dmx_PAT_CheckVersionChange(U8 *pu8Section)
{
    ...
    //In muti-section, parser will limit one channel in one section due to DB size. the total channel number should same as total section number.
    ...
    else
    {
        if(!pstVirtualCh->bChannelFromVCT)
        {
            ...
            {
                printf("[PAT VerChg] u8CurPatItem=%u, u8NumOfPatItem=%u\n", u8CurPatItem, stCurRfCh.u8NumOfPatItem );
                fTableVersionChanged = TRUE;
            }
...

            {
                ...
                for(iu8Loop_i = 0; iu8Loop_i < u8CurPatItem; iu8Loop_i++)
                {
                    ...
                    if(u16ProgramNumber == ((pu8Section[0] << 8) | pu8Section[1]))
                    {
                        ...
                        {
                            printf("u16PmtPid = 0x%x, (((pu8Section[2] & 0x1f) << 8) | pu8Section[3]) = 0x%x\n", u16PmtPid, (((pu8Section[2] & 0x1f) << 8) | pu8Section[3]));
                            fTableVersionChanged = TRUE;
                        }
...
static void MApp_Dmx_PAT_Parse(MS_PAT_ITEM_ATSC*pastPATItem_func, U8 *pu8Section)
{
    ...
    */
    ...
    {
        printf("<Warning> PAT Section number over %u\n", PAT_MAX_SECTION_NUMBER);
        return;
    }
#endif
    ...
    //printf("parse PAT, u16ActualTSid = %u\n", u16ActualTSid);
    pu8Section += 8;
    ...
    if(u8LastSectionNumOfPat)
    {
        ...
        printf("Warning! u8CurSectionItemOfPat(%u) > MAX_VC_PER_PHYSICAL !!\r\n", u8CurSectionItemOfPat);
    }
    else
    {
...
#if ENABLE_PAT_MUTISECTION
    printf("parse PAT, u16ActualTSid = %u, u8CurSectionItemOfPat=%d,  u8CurSecNum=%u\n", u16ActualTSid, u8CurSectionItemOfPat,  u8CurSectionNum);
    ...
    for(u8Loop = 0; u8Loop < u8MaxChNumOfMutiSec; u8Loop++)
    {
        ...
        {
            printf("Warning! u8ChIndex > MAX_VC_PER_PHYSICAL !!\r\n");
            break;
        }
...
U8 MApp_Dmx_GetEitFreeFilter(void)
{
    ...
    {
#if (ENABLE_ATSC_EPG_DB_2016)
        if(GETBIT(stEpgDBInfo.u8EitIsReceivingNew[(u8Loopi/8)],(u8Loopi%8)))
#else
...
#if (ENABLE_ATSC_EPG_DB_2016)
        if(GETBIT(stEpgDBInfo.u8EttIsReceivingNew[(u8Loopi/8)],(u8Loopi%8)))
#else
    ...
    {
        //printf("Free filter = %d, u8EitUsingCount - %d, u8EttUsingCount - %d\n", ((FREE_FILTER_COUNT_EIT)-(u8EitUsingCount+u8EttUsingCount)), u8EitUsingCount, u8EttUsingCount);
...
//printf("_MApp_Dmx_EIT_SetFilter_NewDB(u16_pid=0x%X, u8_table_id=%u)\n", u16_pid, u8_table_id);
    ...
    if(msAPI_DMX_SectionMonitor(&stSectionMonitor, FALSE) == DMX_FILTER_STATUS_OK)
    {
        if (u8_table_id == TID_EIS)
        {
            ...
            {
                //printf("_MApp_EIT_SetBit u8EitIsReceivingNew.....\n");
    ...
    for(u8_loop_i = 0; u8_loop_i < pParse->u8NumOfEvents; u8_loop_i++)
    {
        ...
        if(enEitEvtOp ==EN_PSIP_EIT_EVNET_INSERT)
        {
            ...
            {  //show "Null" or "No Description Available"
                ...
                //strncpy((char *)g_SYS_au8TmpStrBuf, "No Description Available", SYS_TMP_STR_BUF_SIZE);
                Str8_StrCpy_CheckDest((char *)g_SYS_au8TmpStrBuf, "No Description Available", SYS_TMP_STR_BUF_SIZE);

                pSrcEventTitle.pu8Title_content = g_SYS_au8TmpStrBuf;
                ...
                //strncpy((char *)g_SYS_au8TmpStrBuf, "Null", SYS_TMP_STR_BUF_SIZE);
                Str8_StrCpy_CheckDest((char *)g_SYS_au8TmpStrBuf, "Null", SYS_TMP_STR_BUF_SIZE);
    ...
    if (pEvent != NULL)
    {
        if (pEvent->u16DbEttPos == NULL)
        {
            ...
            else
            {
				...
				pSrcEventETT.pu8ETT_content = NULL;//&pu8EpgString[0];
				...
				//strncpy((char *)g_SYS_au8TmpStrBuf, " ", SYS_TMP_STR_BUF_SIZE);
                Str8_StrCpy_CheckDest((char *)g_SYS_au8TmpStrBuf, " ", SYS_TMP_STR_BUF_SIZE);
				pSrcEventETT.pu8ETT_content = g_SYS_au8TmpStrBuf;
...
static void MApp_Dmx_RRT_Parse(MS_REGION5_RATING *pstMgt, U8 *pu8Section)
{
    ...
    for (iu16Loop_i = 0; (iu16Loop_i < u8Num_dimension) && (iu8Buffer_k < REGION5_DIMENSION_NUM); iu16Loop_i++)
    {
        ...
        for( iu8Loop_i = 0;iu8Loop_i < u8Valuesdefined;iu8Loop_i++ )
        {
            ...
            {
#if 0
                strncpy((char *)(pstMgt->stRegin5Dimension[iu8Buffer_k].stRatingText[u8RatingValueLoop]), (char *)(pstMgt->stRegin5Dimension[iu8Buffer_k].stRatingText[u8RatingValueLoop]) + 1);
#else
