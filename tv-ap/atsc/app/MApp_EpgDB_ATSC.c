...
BOOLEAN MApp_EpgDB_GetCurEvent(MS_EPG_EVENT_New * pDstEvent)
{
    ...
    U32 u32EventStartTime = msAPI_Timer_GetSystemTime() + u8GpsUtcOffset;
...
*/
    ...
    if( u8EventCnt && (apstUiEventList[0] != NULL) )
    {
        *pDstEvent = *(apstUiEventList[0]);
        ...
        memcpy(au8EpgUiBuf, stCurrentEvent.au8Title, sizeof(stCurrentEvent.au8Title));
...
void MApp_EpgDB_ContentInit(MS_EPG_VCH_BUF* pVch)
{
    ...
    //strncpy((char *)(&pu8EpgString[0]), "Overflow", EPG_ETT_MAX_STR_LEN);
    Str8_StrCpy_CheckDest((char *)(&pu8EpgString[0]), "Overflow", EPG_ETT_MAX_STR_LEN);
