...
//#include "Si2151_2164B.h"

extern void msAPI_Timer_Delayms(U32 u32DelayTime); //unit = ms


MS_U8 u8PlpIDList[32];
EN_TUNER_MODE m_eMode = E_TUNER_DTV_DVB_T_MODE;//E_TUNER_INVALID;
static AUDIOSTANDARD_TYPE_ m_OtherMode = E_AUDIOSTANDARD_NOTSTANDARD_;



#include "si2151_2164B/Si_I2C/Silabs_L0_Connection.c"
#include "si2151_2164B/Si2151/Si2151_L1_API.c"
#include "si2151_2164B/Si2151/Si2151_L1_Commands.c"
#include "si2151_2164B/Si2151/Si2151_L1_Properties.c"
#include "si2151_2164B/Si2151/Si2151_L2_API.c"
#include "si2151_2164B/Si2151/Si2151_Properties_strings.c"
#include "si2151_2164B/Si2151/Si2151_User_Properties.c"
#include "si2151_2164B/Si2151_Wrapper/SemSi2151.c"
#include "../../cofdm/Si2164B/SiLabs_API_L3_Wrapper.h"



extern SILABS_FE_Context     FrontEnd_Table   [FRONT_END_COUNT];
extern SILABS_FE_Context    *front_end[FRONT_END_COUNT];
extern CUSTOM_Status_Struct  FE_Status   [FRONT_END_COUNT];
extern CUSTOM_Status_Struct *custom_status;

extern L1_Si2151_Context          Si2151_FE_Table   [FRONT_END_COUNT];
extern L1_Si2151_Context         *Si2151_fe;
extern Si2151ConfigParam param;
...
static BOOLEAN Si2151_GetRSSILevel(float *strength_dBm);
...
static BOOLEAN device_tuner_si_2151_init(void)
{
    ...
    //HW reset, silab 2151 is low reset.
    // pull low
    ...
    Si2151_L1_API_Init(api,C_Si2151_LO_ADDRESS);
     /* below power up setting is for successfully excuting Si2151 power up  */
	 /* Si2151_POWER_UP_CMD_EN_XOUT_EN_XOUT, which is to enable XOUT 24MHz clock for another tuner. */
...
if (Si2151_Init(api) != 0 ) {
    ...
    {
      /***wayne*** Enable external AGC1 control pin*/
      if(Si2151_L1_CONFIG_PINS (api,
       ...
       {
             printf(" [Silabs]: ERROR_SETTING_L1_CONFIG_PINS\n");
             retb = FALSE;
	}
	else{
             tuner_init = true;
...
BOOLEAN ATV_SetTune( U32 u32FreqKHz,RFBAND eBand, EN_TUNER_MODE eMode,U8 OtherMode)
{
    ...
    if(tuner_init)
    {
        ...
        {
#if (ENABLE_FM_RADIO)
            case si_tv_system_fm:
                freqHz = (u32FreqKHz * 1000);
        ...
        // Sync from CL1088736 fro ATV scan too slow
        ...
        if( gbTVAutoScanChannelEnable )
        {
            ...
            msAPI_Timer_Delayms(35); //wait 35ms
#endif
            retb = TRUE;
...
BOOLEAN device_tuner_si_2151(void)
{
    m_bDeviceBusy = FALSE;
...
{
    DBG_TUNER(printf("device_tuner_si_2151: Connect\n");)

    if (m_bDeviceBusy == FALSE)
        return FALSE;

    m_bDeviceBusy = TRUE;
printf("device_tuner_si_2151: Connect  m_bDeviceBusy=%d\n",m_bDeviceBusy);
    return TRUE;
}
static BOOLEAN Disconnect()
{
    DBG_TUNER(printf("device_tuner_si_2151: Disconnect\n");)
...
/************************************************************************************************************************
  NAME: Si2151_GetRSSILevel
  DESCRIPTION: Get RSSI from Si2151. RSSI returned by Si2151 is defined with a CW signal to prevent to make assumptions about the analog/digital content.
...
/************************************************************************************************************************
  NAME: Si2151_GetATV_AFC_freq
  DESCRIPTION: Get ATV AFC frequency from Si2151. AFC frequency returned by Si2151 is valid once Si2151 is locked on ATV channel
  Parameter:  Pointer to Si2151 Context (I2C address)
  Returns:    AFC_freq value in Hz, although AFC frequency resolution is in kHz
************************************************************************************************************************/
/*
BOOLEAN Si2151_GetATV_AFC_freq(int* AFC_freqHz)
{
    int return_code = 0;

    if ((return_code=Si2151_L1_ATV_STATUS(api,Si2151_ATV_STATUS_CMD_INTACK_OK))!=0) {
        mcSHOW_HW_MSG((" [Silabs]: ERROR_READING_COMMAND Si2151_ATV_STATUS\n"));
        printf("[Error]%s, %d\n",__FILE__,__LINE__);
        return FALSE;
    }
...
{
    return Si2151_GetRSSILevel(f_strength);
}
    ...
    if(TV_SYS == si_tv_system_invalid)
    {
        switch (eNetworktype)
        {
		 ...
		 api->prop->dtv_config_if_port.dtv_agc_source    =     Si2151_DTV_CONFIG_IF_PORT_PROP_DTV_AGC_SOURCE_AGC1_3DB;
                if (Si2151_L1_SetProperty2(api, Si2151_DTV_CONFIG_IF_PORT_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si2151_DTV_LIF_OUT_PROP\n"));
                }
		#endif
    ...
    else
    {
        switch (TV_SYS)
        {
            ...
            case si_tv_system_fm:
                ...
                api->prop->atv_lif_freq.offset = C_Si2151_IF_CENTER_FM;
                if (Si2151_L1_SetProperty2(api, Si2151_ATV_LIF_FREQ_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si2151_ATV_LIF_FREQ_PROP\n"));
                }
...
#endif
            case si_tv_system_bg:
               // api->prop->atv_video_mode.color = Si2151_ATV_VIDEO_MODE_PROP_COLOR_PAL_NTSC;
                api->prop->atv_video_mode.invert_spectrum = Si2151_ATV_VIDEO_MODE_PROP_INVERT_SPECTRUM_INVERTED;
                if (Si2151_L1_SetProperty2(api, Si2151_ATV_VIDEO_MODE_PROP) != 0)
                {
                    mcSHOW_HW_MSG((" [Silabs]: ERROR_SETTING_PROPERTY Si2151_ATV_VIDEO_MODE_PROP\n"));
...
#if 0
int Si2151_printf(const char *format, ...)
{
    //format = format;
     printf(format);
    return 0;
...
BOOLEAN devTuner_Std_set(EN_TUNER_MODE eMode, AUDIOSTANDARD_TYPE_ OtherMode)  //record current standard
{
    BOOLEAN bRet = TRUE;
    m_eMode = eMode;
    m_OtherMode = OtherMode;
...
//extern signed int   SiLabs_API_lock_to_carrier          (SILABS_FE_Context *front_end, unsigned char standard, signed int freq, signed int bandwidth_Hz, unsigned char stream, unsigned int symbol_rate_bps, unsigned char constellation, unsigned char polarization, unsigned char band, signed int data_slice_id, signed int plp_id, unsigned char T2_lock_mode)
/*
...
void devDigitalTuner_SetFreq ( double Freq,  RF_CHANNEL_BANDWIDTH eBandWidth)
{
    ...
    SiLabs_API_lock_to_carrier(Si2151_fe,std,Freq, );
#endif
    Freq=Freq;
    eBandWidth=eBandWidth;
    printf("\r\n devDigitalTuner_SetFreq");
    //CPU_PAUSE();
