...
#define R842_Delay_MS(x) msAPI_Timer_Delayms(x)
...
// I2C functions
//============================================================================//
BOOL UserWrittenI2CRead(UINT32 AddrSize, UINT8* pAddr,UINT32 ReadLen, UINT8* pData);
BOOL UserWrittenI2CWrite(UINT32 AddrSize, UINT8* pAddr,UINT32 WriteLen, UINT8* pData);
UINT8 Rafael_Convert(UINT8 InvertNum)
{
    UINT8 ReturnNum = 0;
    UINT8 AddNum    = 0x80;
    UINT8 BitNum    = 0x01;
...
{
    return UserWrittenI2CWrite(1, &I2C_Info->RegAddr, I2C_Info->Len, I2C_Info->Data);
}

BOOL I2C_Read_Len(I2C_LEN_TYPE *I2C_Info)
{
    UINT8 i;
    BOOL bRet = UserWrittenI2CRead(0, NULL, I2C_Info->Len, I2C_Info->Data);

    for(i = 0; i < I2C_Info->Len; i++)
    {
        I2C_Info->Data[i] = Rafael_Convert(I2C_Info->Data[i]);
    }
...
{
    return UserWrittenI2CWrite(1, &I2C_Info->RegAddr, 1, &I2C_Info->Data);
}
...
R842_SysFreq_Info_Type R842_SysFreq_Sel(R842_Standard_Type R842_Standard,UINT32 RF_freq)
{
    ...
    memset(&R842_SysFreq_Info,0,sizeof(R842_SysFreq_Info));
...
R842_ErrCode R842_InitReg(void)
{
	...
	if(I2C_Write_Len(&R842_I2C_Len) != R842_Success)
		return R842_Fail;
	...
	if(I2C_Write(&R842_I2C) != R842_Success)
		return R842_Fail;
...
R842_ErrCode R842_TF_Check(void)
{
	...
	R842_I2C_Len.RegAddr = 0;
	R842_I2C_Len.Len = 2;    
	if(I2C_Read_Len(&R842_I2C_Len) != R842_Success)
	{
		I2C_Read_Len(&R842_I2C_Len);
	}
...
R842_ErrCode R842_GetIfRssi(UINT32 *VgaGain)
{
	...
	Dlg_I2C_Len.Len = 2;
	if(I2C_Read_Len(&Dlg_I2C_Len) != R842_Success) // read data length
	{
