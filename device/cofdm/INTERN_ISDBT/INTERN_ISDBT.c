...
#define ISDBT_TUNER_IF      		4000
#elif (FRONTEND_TUNER_TYPE == RAFAEL_R828_TUNER)
#define ISDBT_TUNER_IF      		4063
#elif ((FRONTEND_TUNER_TYPE == RDA5158_TUNER) || (FRONTEND_TUNER_TYPE == SILAB_2151_TUNER)||(FRONTEND_TUNER_TYPE == SILAB_Si2151_2164B_TUNER))
...
#ifdef DEMOD_ISDBT_BUF_ADR
    #if DEMOD_ISDBT_BUF_ADR != DEMOD_DVBT2_BUF_ADR
    #error "DEMOD_ISDBT_BUF_ADR != DEMOD_DVBT2_BUF_ADR"
    #endif
    #define DMD_ISDBT_ADR   DEMOD_ISDBT_BUF_ADR
#else
    #define DMD_ISDBT_ADR   DEMOD_DVBT2_BUF_ADR
#endif
#endif

#define ISDBT_TDI_START_ADDR (DMD_ISDBT_ADR/16)   // System must allocate a 7MB (Marlon 3.7MB) memory space starting from this (128-bit)address


//static BOOL s_bI2C_FastMode = false;

float INTERN_ISDBT_GetSNR(void);
...
BOOLEAN INTERN_ISDBT_Power_On_Initialization(void)
{
    ...
    printf("INTERN_ISDBT_Power_On_Initialization\n");
...
BOOLEAN INTERN_ISDBT_Lock(COFDM_LOCK_STATUS eStatus)
{
...
BOOLEAN INTERN_ISDBT_Config(RF_CHANNEL_BANDWIDTH BW, BOOLEAN bSerialTS, BOOLEAN bPalBG, BOOLEAN bLPSel)
{
    BW = BW;
    bSerialTS = bSerialTS;
    bPalBG = bPalBG;
...
BOOLEAN INTERN_ISDBT_Active(BOOLEAN bEnable)
{
    return MDrv_DMD_ISDBT_SetConfig(bEnable);
}

BOOLEAN INTERN_ISDBT_Serial_Control(BOOLEAN bEnable)
{
...
FUNCTION_RESULT INTERN_ISDBT_GetSignalStrength(WORD *strength)
{
...
FUNCTION_RESULT INTERN_ISDBT_GetSignalQuality(WORD *quality)
{
    FUNCTION_RESULT ret = E_RESULT_SUCCESS;

    *quality = MDrv_DMD_ISDBT_GetSignalQuality();
    //printf("INTERN_ISDBT_GetSignalQuality = %ld\n",  *quality);
    if ( *quality > 100 )
        ret = E_RESULT_FAILURE;
    return ret;
}
BOOLEAN INTERN_ISDBT_WriteReg(MS_U16 RegAddr, MS_U8 u8Data)
{
    return MDrv_DMD_ISDBT_SetReg(RegAddr, u8Data);
}
BOOLEAN INTERN_ISDBT_ReadReg(MS_U16 RegAddr, MS_U8* pu8Data)
{
    return MDrv_DMD_ISDBT_GetReg(RegAddr, pu8Data);
}

BOOLEAN INTERN_ISDBT_Exit ( void )
{
    return MDrv_DMD_ISDBT_Exit();
}

void INTERN_ISDBT_Power_ON_OFF(MS_U8 bPowerOn)
{
    bPowerOn = bPowerOn;
}

BOOLEAN INTERN_ISDBT_Power_Save(void)
{
...
/************************************************************************************************
  ...
  Remark:
*************************************************************************************************/
void INTERN_ISDBT_PAD_TS1_Enable(BOOLEAN flag)
{
