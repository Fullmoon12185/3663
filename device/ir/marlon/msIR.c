...
#if ENABLE_IR_BIN
extern U8 g_u8IR_HEADER_CODE0;
extern U8 g_u8IR_HEADER_CODE1;
#endif
...
#if ENABLE_IR_BIN
    msIR_WriteByte(IR_CCODE_L, g_u8IR_HEADER_CODE0);
    msIR_WriteByte(IR_CCODE_H, g_u8IR_HEADER_CODE1);
#else
...
static BOOLEAN msIR_GetRawKey(U8 *pu8key,U8 *pu8flag,BOOLEAN bChkCCode)
{
    ...
    *pu8flag = (msIR_ReadByte(IR_RPT_FIFOEMPTY) & 0x1)? 1 : 0;
    ...
    if (u8IRRawModeDone)
    {
        ...
        {
        #if ENABLE_IR_BIN
            if ( (u8IRRawModeBuf[0] == g_u8IR_HEADER_CODE0) && (u8IRRawModeBuf[1] == g_u8IR_HEADER_CODE1) )
        #else
    ...
    *pu8flag = g_u8IRRpt;
    ...
    if(g_u8IrRc5Flag & IR_RC5_DETECT_END)
    {
        ...
        if ((g_u16IrRc5Data & 0xF000) == 0x3000)
        {
          ...
          #if ENABLE_IR_BIN
            if (u8System == g_u8IR_HEADER_CODE0)
          #else
    ...
    {
    #if ENABLE_IR_BIN
        if (((g_u16IrKonDecode >> 8) & 0xFF) != g_u8IR_HEADER_CODE0)
    #else
...
//       g_bIrHold = TRUE;
    ...
    #if 1
//SH_IR_REPEAT_THRESHOLD is a threshold to control user press RC frequently or keep press RC
//SH_IR_REPEAT_DELAY is using for controling first RC repeat delay
...
//+merge Mint v1.15<<<
            ...
            {
                 if((CurrentTime-KeyOffTimeOutCount)>500)
                 {
                     SetToStandbyMode=TRUE;
                 }
                 if(SetToStandbyMode)
