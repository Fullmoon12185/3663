...
BOOLEAN msIR_Is_GetAutoIRCode(U32 u32IrNecDecodeData)
{
    U8 u8header = (u32IrNecDecodeData & 0xFF);
    U8 u8Opcode = ((u32IrNecDecodeData >> 8)& 0xFF);
    U8 u8Data = ((u32IrNecDecodeData >> 16)& 0xFF);
    U8 u8CSC = ((u32IrNecDecodeData >> 24)& 0xFF);
    g_u32AutoIrDecodeData = 0;

    if(IR_IS_AUTOIR_HEADER_CODE(u8header) && (u8CSC == (U8)(u8header + u8Data + u8Opcode)))
    {
        g_u32AutoIrDecodeData = u32IrNecDecodeData;
        g_bExecuteAUTOIrResponse = TRUE;
        return TRUE;
    }
    else
    {
        g_bExecuteAUTOIrResponse = FALSE;
        return FALSE;
    }
}
#endif
...
#elif(IR_MODE_SEL == IR_TYPE_SWDECODE_NEC_MODE)
{
    ...
    if (g_u8IrNecFlag & IR_NEC_DETECT_END)
    {
        ...
        else
        {
            ...
            if(msIR_Is_GetAutoIRCode(g_u32IrNecDecodeData))
            {
                g_u8IrPreKey = IRKEY_DUMY;
                g_u8IrNecFlag &= (~IR_NEC_DETECT_END);
                return MSRET_ERROR;
            }
            else if ( !(IR_IS_USER_HEADER_CODE(u16Temp&0x0F) || IR_IS_FACT_HEADER_CODE((u16Temp>>8) &0x0F)) )
            {
                g_u8IrPreKey = IRKEY_DUMY;
                g_u8IrNecFlag &= (~IR_NEC_DETECT_END);
                return MSRET_ERROR;
...
BOOLEAN MDrv_Power_CheckPowerOnSWIRKey(void)
{
    ...
    if (msIR_GetIRKeyCode(&u8Key, &u8Flag))
    {
        if (u8Key == IRKEY_POWER)
            return TRUE;
        //else if (u8Key == IRKEY_GEO_POWER)      //Karl del temp 12/02/2009
            //return TRUE;
